using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations.Schema;

namespace ApiAll.Model.pos
{
    [Table("pos_client_schot")]
    public class PosClientSchot : PosBaseModel
    {
        public double qty { get; set; } = 0.0;
        public double real_qty { get; set; } = 0.0;
        public DateTime reg_date { get; set; } = DateTime.Now;
        public PosClient client { get; set; }
        public long PosClientid { get; set; }

        [System.Text.Json.Serialization.JsonIgnore]
        [Newtonsoft.Json.JsonIgnore]
        public List<PosClientSchotItem> itemList { get; set; }
        public String note { get; set; }
        [NotMapped]
        public String reg_date_str => reg_date != null ? ((DateTime)reg_date).Date.ToString("dd-MM-yyyy") : "";
        [NotMapped]
        public String qty_price_str => String.Format("{0:N}", qty);
        [NotMapped]
        public String real_qty_price_str => String.Format("{0:N}", real_qty);
        [NotMapped]
        public String client_name => client != null ? client.fio : "";

    }
}

