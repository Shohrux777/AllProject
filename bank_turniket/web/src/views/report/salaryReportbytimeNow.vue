...
export default {
  ...
  methods:{
    ...
    async saveEditedWorkedTime(index, item){
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/SkudMyUserinfoes/replaceMyCheckinoutIoByRange", {
          method: "POST",
          headers: {
    ...
    applyManualIoToItemList(item, firstIn, lastOut, doorName, userId){
      ...
      item.list = item.list.filter((row) => {
        if (!row) return false;
        if (!(row.checktype === 'I' || row.checktype === 'O')) return true;
    ...
    async fetch_user_details_input(id, date){
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/SkudMyUserinfoes/getPaginationBy0404IchkiQarovul?page=0&size=1000&date_b=" + start + '&user_id=' + id);
        const data = await response.json();
        this.loading = false;
    ...
    async fetch_user_work_use(id, date){
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/SkudMyUserinfoes/getReportWithoutSmenaPaginationByAllUserAndOylik0404IchkiQarovulWithUserId?page=0&size=1000&begin_date=" + start + '&end_date=' + end + '&user_id=' + id);
        const data = await response.json();
        this.loading = false;
        console.log('userIdReport');
        console.log(data.items_list);
    ...
    async submit(){
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/SkudMyUserinfoes/getReportWithoutSmenaPaginationByAllUserAndOylik0404IchkiQorovulxona?page=0&size=1000&begin_date=" + start + '&end_date=' + end);
        const data = await response.json();
        this.loading = false;
        this.reportList = [];
