...
export default {
    ...
    methods:{
      ...
      async deluser_day_data(del_id, user_name){
        if (confirm("Ты уверен? Удаляет Заметка" + user_name) == true) {
            console.log(del_id)
            try{
                ...
                const response = await fetch(this.$store.state.hostname + "/SkudUserNote/" + del_id, requestOptions);
      ...
      async submit(){
        ...
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + "/SkudUserNote/getPagination?page=0&size=200");
