              ...
              <MDBTable class="align-middle mb-0 bg-white">
                  ...
                  <tr v-for="(row, index) in filteredList" :key="index">
                    <td>{{index+1}}</td>
                      ...
                      <td>{{row.dept}}</td>
                      <td @click="blockUser(row)" class="text-center text-danger" 
...
<script>
import {
    MDBIcon,MDBInput, MDBBtn, MDBTable, MDBBadge,MDBSwitch
  } from 'mdb-vue-ui-kit';
...
export default {
    ...
    methods:{
      ...
      async blockUser(item){
        ...
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + "/SkudMyUserinfoes/getBlokUsersListForAllDoorsWithUpdateGr?user_id_list=" + item.userid);
          const data = await response.json();
          this.loading = false;
          console.log(data)
          // this.reportList = data.items_list;
      ...
      async updateSetting_id(){
        try{
          const response = await fetch(this.$store.state.hostname + "/SkudSettings/1");
          const data = await response.json();
          console.log('data')
          console.log(data)
          this.switch1 = data.status_1;
        }
        catch(error){
          console.log(error)
        }
      ...
      async changeSwitch(){
          ...
          try{
            ...
            body: JSON.stringify({
              "id": 1,
              "status_1" : !this.switch1,
              })
            };
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + "/SkudSettings", requestOptions);
            const data = await response.json();
            this.loading = false;
            console.log(data)
          }
