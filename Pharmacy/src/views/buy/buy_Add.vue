    ...
    <div class="" v-else>
      ...
      <main style="border: 0.1px solid white; border-radius: 7px;" class="grey lighten-5">
            ...
            <div :class="{'applied': sklad_submitted}" class="container-fluid mt-3  bgColorBack p-4" style=" border-radius: 7px;">
              ...
              <div class="">
                  ...
                  <mdb-col col="6" class="text-right">
                    ...
                    <mdb-btn class="mr-1 px-3 py-2 text-white" style="font-size:12px" color="success" darkWaves tag="a" floating @click="productAddBtn = true"
                    icon="plus"  size="sm">{{$t('add_product')}}</mdb-btn>
                  ...
                  <mdb-tbl class="fixed-column" >
                    ...
                    <mdb-tbl-body>
                      <tr  scope="row" v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" :width="row.size">
                        ...
                        <th v-show="false"> <input v-model="row.optom_price"  type="number" class="form-control form-control-sm" >  </th>
        ...
        <Toast ref="message"></Toast>
          ...
          <!-- Central Info Modal -->
      <mdb-modal  :show="productAddBtn"  @close="productAddBtn = false" size="xl" info>
        <!-- <mdb-modal-header>
    ...
    data(){
      return{
        ...
        datasource :
        {
          rows :[],
          // real_time
          columns: ['product','soni','unit','contains_number','price','precent','buyed_price','nds','skidka','end_time','end_time','summ'],
        }
      }
    },
    async mounted(){
     ...
     async close_product(last_product)
      {
        ...
        this.datasource.rows.unshift({
            main_item_id : last_product.name.id,
            product_id : last_product.name.id,
            product_name : last_product.name.name,
            soni : 0,
            unit: 0,
            numberDrag: last_product.name.contains_number_in_pack,
            buyed_price : last_product.name.buyed_price,
            prosent : last_product.name.percentage,
            prosentNDS: 0,
            persantage_discount: 0,
            price : last_product.name.price,
            date: '',
            summ: 0,
            optom_price: 0,
