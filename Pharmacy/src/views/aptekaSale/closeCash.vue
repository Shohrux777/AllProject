  ...
  <div class="addProductQtyAcceptCash"  v-on:click.self="close">
    <div class="p-4" style="height: 450px;">
      <div class="acceptBox px-3 py-2 pb-4 text-center">
        ...
        <div class="d-flex mt-2">
          <div class="skidka " style="width:35%">
            <div style="position:relative;" class="text-left text-success">
              <h6 class="m-0 p-0 mt-2">{{$t('cash')}}</h6>
            </div>
        ...
        <div class="d-flex mt-2">
          <div class="skidka " style="width:35%">
            <div style="position:relative;" class="text-left text-info">
              <h6 class="m-0 p-0 mt-2">UzCard</h6>
            </div>
        ...
        <div class="d-flex mt-2">
          <div class="skidka " style="width:35%">
            <div style="position:relative;" class="text-left text-warning">
              <h6 class="m-0 p-0 mt-2">{{$t('skidka')}}</h6>
            </div>
        ...
        <div class="d-flex mt-2">
          <div class="skidka " style="width:35%">
            <div style="position:relative;" class="text-left text-danger">
              <h6 class="m-0 p-0 mt-2">{{$t('creditd_summ')}}</h6>
            </div>
          </div>
          <div class="price ml-4 "  style="width:65%; position:relative;">
            <div style="position:relative; height: 38px;" class="border rounded text-right ">
    ...
    async getAllSumm(){
      ...
      try{
        ...
        if(res_data.total_sum == null){
          ...
          this.totalIn = (res_data.total_sum - res_data.online_sum - res_data.discount_sum).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.cashInString = res_data.cash_sum.toFixed(0).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.uzcardInString = res_data.card_sum.toFixed(0).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.humoInString = res_data.humo_sum.toFixed(0).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.clickInString = res_data.discount_sum.toFixed(0).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.onlineInString = res_data.online_sum.toFixed(0).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
        }
