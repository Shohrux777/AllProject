...
<div>
  ...
  <div v-else class="aptekaSale" >
    <div class="saleHeader border-bottom d-flex justify-content-between align-items-center"  v-on:click.capture="focusBarcode">
      <div class="d-flex">
        <router-link v-if="access_type == 1" to="/getSaledSummOfKassirListBeatweenDateReport" >
          <h5 class="m-0 ml-3 d-flex align-items-center" style="padding: 7px 0px; font-size: 15.5px;">
              <mdb-icon icon="bars" style="font-size: 15px;"  class="mr-2 "/>
              {{$t('report')}}</h5>
          </router-link>
    ...
    <div class="allPage" >
                ...
                <table  class="myTablesforSale" >
                  ...
                  <tbody>
                    <tr v-for="(item,i) in searchBarcodeData" :key="i" class="searchBarcodeHover" @click="clickBarcodeData(item)" 
                      ...
                      <td style="font-weight: bold;">{{item.real_qty.toFixed(1)}}</td>
                      <td>{{item.product.buyed_price}}</td>
                      <td>{{item.expired_date.slice(0,10)}}</td>
                      <td>{{item.product.publisher_name}}</td>
      ...
      <div class="SummaBtnRight border"  v-on:click.capture="focusBarcode">
        ...
        <div class="d-flex" v-on:click.capture="focusBarcode">
          <div class="w-50">
            <div class="text-center mt-1 linkMain" style="padding: 9.2px 0;" @click="$router.push('/checkList')">
          ...
          <div class="w-50">
            ...
            <mdb-btn color="warning" style="font-size:11px; padding: 12px 0;" @click="clickPay" class="m-0  mt-1 w-100"> {{$t('skidka')}}</mdb-btn>
            <mdb-btn color="indigo" style="font-size:11px; padding: 12px 0;" @click="onlinePay" class="m-0  mt-1 w-100" > {{$t('creditd_summ')}}</mdb-btn>
    ...
    <div class="addProductQtyAccept" v-show="showAccept" v-on:click.self="closeChoosenProduct">
      <div class="acceptBox px-3 py-2">
        ...
        <div class="d-flex mt-3">
          <div class="skidka " style="width:35%">
            <div style="position:relative;" >
              <input type="text" v-model="empty_nol" disabled class="form-control form-control-sm border mt-2 text-right" style="border:none; outline:none;font-weight:bold;" >
        ...
        <div class="reportproduct  mt-3 border d-flex pl-3"  style="position:relative;">
          <h4 v-if="dataAccept.contains_number_in_pack>1" class="font-weight-bold p-0 m-0 my-1"><span>{{dataAccept.ostatka_qty}}</span> <span>уп . </span> <span class="ml-2">{{dataAccept.ostatka_unit}}/{{dataAccept.contains_number_in_pack}}</span> <span>шт</span></h4>
          <h4 v-else class="font-weight-bold p-0 m-0 my-1"><span>{{dataAccept.ostatka_qty}}</span> <span>{{ dataAccept.measurement }} </span> </h4>
            <small style="position:absolute; top:-16px; left:3px;">
              {{$t('reportproduct')}}
            </small> 
        </div>
        <div class="d-flex align-items-center mt-3" style="position:relative;">
          ...
          <div class="skidka pl-2" style="width:20%">
            <div style="position:relative;">
              <input v-model="acceptUnit" :disabled="dataAccept.contains_number_in_pack<=1" v-on:keyup.37 = "ArrowRightS" ref="acceptUnit" v-on:keyup.13 = "AddAcceptPage"
    ...
    <div  class="d-flex justify-content-center align-items-center  shadow" :class="{'InfoIcoFixedOpenDrag': showSendingDrug, 'InfoIcoFixedDrag': !showSendingDrug}"  @click="show_patient_come_drug">
      <mdb-icon icon="capsules" class="text-white ml-1" style="font-size: 22px;" />
    </div>
    <div class="infoPatientIdDrug" :class="{'infoPatientIdDrug': showSendingDrug, 'infoPatientIdNo': !showSendingDrug}">
      <div class="users_patient_table container-fluid">
        <div class="hos_table_patient border" v-show="!showSendingDrug_Patient">
          <table  class="myTableHospital  px-2 w-100" >
            <thead>
              <tr  class="py-2 headerPatientList">
              <th style="width: 30px;"><p class="py-2 m-0 ml-2">№</p></th>
              <th style="width: 250px;"><p class="py-2 m-0">{{$t('patient')}}</p></th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item,i) in patient_list" :key="i" class="searchBarcodeHoverPatient border-bottom py-4" @click="choosePatient(item.bemor_id)" >
                <td ><p class="py-2 m-0 ml-2">{{i+1}}</p></td>
                <td ><p class="py-2 m-0">{{item.bemor_fio}}</p></td>
              </tr>
            </tbody>
          </table>
        </div>
        <div class="box_drug_sell_accept" v-show="showSendingDrug_Patient">
          <div class="hos_table_patient_drug border">
            <table  class="myTableHospital  px-2 w-100">
              <thead>
                <tr  class="py-2 headerPatientList">
                <th style="width: 30px;"><p class="py-2 m-0 ml-2">№</p></th>
                <th style="width: 250px;"><p class="py-2 m-0">{{$t('product')}}</p></th>
                <th ><p class="py-2 m-0">{{$t('soni')}}</p></th>
                <th ><p class="py-2 m-0">{{$t('unit_qty')}}</p></th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item,i) in patient_drug_list_for" :key="i" class="searchBarcodeHoverPatient border-bottom py-4">
                  <td ><p class="py-2 m-0 ml-2">{{i+1}}</p></td>
                  <td ><p class="py-2 m-0">{{item.posProduct.name}}</p></td>
                  <td ><p class="py-2 m-0">{{item.qty}}</p></td>
                  <td ><p class="py-2 m-0">{{item.real_qty}}</p></td>
                </tr>
              </tbody>
            </table>
          </div>
          <div class="send_pharmasy text-right">
              <mdb-btn color="danger" @click="showSendingDrug_Patient = false" class="m-0 py-2 mt-1 ml-2"> Close </mdb-btn>
              <mdb-btn color="primary" @click="accept_drug" class="m-0 py-2 mt-1 ml-2"> Accept </mdb-btn>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      empty_nol: 0,
      access_type: localStorage.AccessType,

      showSendingDrug: false,
      showSendingDrug_Patient: false,
      patient_list: [
        
      ],

      patient_drug_list_for: [
  ...
  methods: {
    ...
    async choosePatient(id){
      this.showSendingDrug_Patient = true;
      try{
        const response = await fetch(this.$store.state.hostname + "/HospitalPatientDrugs/getPaginationWithPatientIdNotGetDrugsList?page=0&size=150&patient_id=" + id);
        const data = await response.json();
        console.log('data items drugs')
        console.log(data.items_list)
        if(data.items_count){
          this.patient_drug_list_for =data.items_list;
        }
      }
      catch{
        console.log('Oshibka')
    ...
    async accept_drug(){
      let id_list = [];
      for(let i=0; i<this.patient_drug_list_for.length; i++){
        id_list.push(this.patient_drug_list_for[i].id)
      }
      try{
        ...
        // this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/HospitalPatientDrugs/addPostListDrugsSetStatusTrue", requestOptions);
        const data = await response.json();
        // this.loading = false;
        console.log('data payments')
        console.log(data)
        this.$refs.message.success('Added_successfully');
        this.showSendingDrug_Patient = false;
        await this.fechPatientAllList();
    ...
    async show_patient_come_drug(){
      this.showSendingDrug = !this.showSendingDrug;
      await this.fechPatientAllList();
    },
    async fechPatientAllList(){
      console.log('ishladi kirdi')
      try{
        const response = await fetch(this.$store.state.hostname + "/HospitalPatientDrugs/doriOlmaganBemorlarRoyixati");
        const data = await response.json();
        console.log('data items drugs')
        console.log(data)
        if(data[0].bemor_id){
    ...
    changePriceSaled(){
      ...
      if(this.dataAccept.contains_number_in_pack>1){
        ...
        this.dataAccept.price = this.dataAccept.saledPrice * this.acceptQty;
        this.UnitSumString = new Intl.NumberFormat().format(this.dataAccept.price)
        
      }
    },
    changeOptimPrice(price){
      this.unitPrice = new Intl.NumberFormat().format(price)
      this.dataAccept.saledPrice = parseFloat(price)
      if(this.dataAccept.contains_number_in_pack>1){
        this.dataAccept.price = ((this.acceptQty * this.dataAccept.saledPrice) + (parseFloat((this.acceptUnit*this.dataAccept.saledPrice)/(this.dataAccept.contains_number_in_pack)))).toFixed(2)
        this.UnitSumString = new Intl.NumberFormat().format(this.dataAccept.price)
      }
    ...
    // <--  Productni tanlab sonini yozadigan oyna ochadi start-->
      clickBarcodeData(data){
        ...
        if(data.product.contains_number_in_pack>1){
          ...
          this.dataAccept.ostatka_qty = parseFloat(data.real_qty) - parseFloat(chooseQty);
          this.dataAccept.ostatka_unit = 0;
          console.log(this.dataAccept.ostatka_unit)
        }
        ...
        this.dataAccept.realPriceSaled = data.product.buyed_price;
        this.dataAccept.price = 0;
        this.dataAccept.optom_price = data.product.optom_price;
...
.searchBarcodeHoverPatient:hover{
    background: seagreen;
    color:seashell;
    cursor: pointer;
  }
  .searchBarcodeHoverPatient{
