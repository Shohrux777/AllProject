  ...
  <div class="result_lab result_lab22">
    ...
    <div v-else class="result_lists container-fluid d-flex">
      <div class="w-100 mx-4">
        <div class=" itemResult border p-3 mt-5 rounded bgorange " v-for="(item,i) in result_list" :key="i" v-show="item.analiz_hos_id == analiz_hos_blank_id">
          <div class="d-flex justify-content-center border-bottom">
            <h5 style="color:#4B515D;">Дата результата: <span class=" ml-1" style="color:#FF8800;"> {{item.update_date.slice(0,10)}} <span class="ml-2"></span>  {{item.update_date.slice(11,19)}}</span></h5>
          </div>
          <div class="d-flex justify-content-between border-bottom">
            <h4  style="color:#4B515D;" class="m-0 py-2">Пациент: <span  style="color:#0d47a1;" class=" ml-1">{{item.patients.FIO}}</span></h4>
            <h5  style="color:#4B515D;" class="m-0 py-2">Врач: <span style="color:#0d47a1;" class=" ml-1">{{item.extra1}}</span></h5>
          </div>
          <div class="d-flex justify-content-center border-bottom py-3" style="position:relative;">
            <span style="color:#757575; font-size:25px;">Анализ: <span class=" ml-1" style="color:#1b5e20;"> {{item.name_title}}</span></span>
      ...
      <!-- <h1>Xulosalar</h1> -->
      <div class="w-25 mx-4" v-show="false">
...
export default {
  ...
  data() {
    return {
      ...
      admin: true,
      patient_id:null,
      analiz_hos_blank_id: localStorage.hos_id,
  ...
  methods: {
      ...
      async updateHosBlank(){
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + '/HospitalAnalizDynamicResults/getPaginationByHosBlankId?page=0&size=100&hos_id=' + localStorage.hos_id)
          const data = await response.json();
          this.loading = false;

          console.log('result_lab')
          console.log(data)
    ...
    async search_func(){
      ...
      async choosePatientID(id){
        ...
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + '/HospitalAnalizDynamicResults/getPaginationByPatientId?page=0&size=10000&patient_id=' + id)
          const data = await response.json();
          this.loading = false;
      ...
      printed(id){
        console.log(id)
        this.$router.push('/lab_printed/' + id)
      },
      printedTash(id){
