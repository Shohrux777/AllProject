   ...
   <loader v-if="loading"/>
    ...
    <form v-on:submit.prevent="submit" >
        ...
        <div class="userEnter px-3 pb-2">
          <div class="row">
            ...
            <div class="col-12 pt-0 m-0">
              ...
              <mdb-input size="sm" label="ID" type="number" style="margin-top: -7px;"  v-model="id_patient" outline/>
      ...
      <div class="row">
       ...
       <div style="position:sticky; bottom:-10px; width:100%;" class=" bg-white">
          <!-- <div v-show="!next" class="text-right border-top mt-2" >
  ...
  async mounted(){
    ...
    methods: {
      ...
      getBemorId(i, item){
        ...
        this.workAdress =  item.workAddress
      ...
      async nextpage(){
        ...
        try{
          ...
          this.tel = phoneNum;
          console.log(this.workAdress)
          if(this.workAdress == '' || this.workAdress == null){
            console.log('entered workA')
            this.workAdress = this.makeid(6);
          }
          if(this.$v.$invalid )
          {
            this.$v.$touch();
      ...
      async update(data){  
        ...
        this.workAdress =  data.workAddress
        if(data.polType == 1){
          this.men = true;
          this.girl = false;
        }else{
        ...
        this.name = data.fio;
        this.bemorId = data.id
        var temp = ''
        for(let i=4;i<data.phoneNumber.length; i++){
          temp += data.phoneNumber[i]
        }
        var y = temp.replace(/\D/g, '').match(/(\d{0,2})(\d{0,3})(\d{0,4})/);
        this.tel = !y[2] ? y[1] : '(' + y[1] + ') ' + y[2] + (y[3] ? '-' + y[3] : '');

        // this.tel = data.phoneNumber;
        this.time1 = data.bornDate.slice(0,10);
        this.passport = data.passportSerialNumber;
        this.workAdress =  data.workAddress
        ...
        await this.fetch_contragent()
        if(this.get_contragent_list.rows.length != 0){
          console.log('this.get_contragent_list')
          console.log(this.get_contragent_list)
          this.contragent_name = this.get_contragent_list.rows[0].name;
          this.contragent_id = this.get_contragent_list.rows[0].id;
          this.patient_info.contragent_name = this.get_contragent_list.rows[0].name;
          this.patient_info.contragent_tel = this.get_contragent_list.rows[0].phoneNumber;
          this.patient_info.contragent_id = this.get_contragent_list.rows[0].id;
        }
      },
      ...
      async submit(){
        if(this.id_patient){
          try{
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + '/Patients/' + this.id_patient)
            const data = await response.json()
            this.loading = false;
            console.log(data);
            this.getBemorId(100, data);
          }
          catch(error){
            console.log(error)
          }
