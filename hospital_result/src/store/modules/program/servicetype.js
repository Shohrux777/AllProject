    ...
    mutations: {
        ...
        choosenServiceType(state, option){
            ...
            if(option.data.status == false){
                ...
                for(let i=0; i<state.service_no_chosen_list.length; i++){
                    if(state.service_no_chosen_list[i].id == option.data.id){
                        state.service_no_chosen_list[i].status = true;
                        state.serviceChosenList.push(state.service_no_chosen_list[i])
                        return;
                    }
                }
                ...
                for(let i=0; i<state.service_no_chosen_list.length; i++){
                    if(state.service_no_chosen_list[i].id == option.data.id){
                        state.service_no_chosen_list[i].status = false;
                        return;
                    }
                }
