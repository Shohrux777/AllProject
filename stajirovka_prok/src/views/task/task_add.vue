            ...
            <div class="col-9">
                <form @submit.prevent="submit_add" class="container">
                    ...
                    <MDBTextarea label="Tavsif" 
                        class="mt-3"
                        size="sm"
                        rows="2" v-model="task.description" />
        ...
        var img = document.getElementById('prewImage');
          ...
          try {
            ...
            const response = await axios.put(
              this.$store.state.hostname + '/api/admin/tasks/' + this.select_data.id,
              this.task,
              {
                headers: {
                  Authorization: `Bearer ${token}`
