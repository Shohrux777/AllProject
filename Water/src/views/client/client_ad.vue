<template>
  <div>
    ...
    <div class="row">
        ...
        <div class="client_add px-5 pt-4">
          <form @submit.prevent="submit">
            <div class="row">
              ...
              <div class="col-12 col-sm-12 col-md-7 col-lg-7 px-2">
            ...
            <div class="mt-4 row" v-for="(item,i) in phoneList" :key="i+50">
              <div class="col-12 col-sm-12 col-md-5 col-lg-5">
                <p class="p-0 m-0 mt-2" style="font-size: 14px;">{{$t('phoneNumber') + (i+1)}}</p>
              </div>
              <div class="col-12 col-sm-12 col-md-7 col-lg-7 px-2" style="position:relative;">
            ...
            <div class="mt-4 row" v-show="false">
              <div class="col-12 col-sm-12 col-md-5 col-lg-5">
            ...
            <div class="mt-4" style="position:relative;" v-for="(list,index) in addressList" :key="index">
              ...
              <div class="row mt-4">
                ...
                <div class="col-12 col-sm-12 col-md-7 col-lg-7 px-2" style="position:relative;">
                  ...
                  <div class="added bg-danger rounded" v-show="index!=0" style="position:absolute; right:-30px; top:67px;" @click="delAddress(index)">
                    <mdb-icon icon="trash p-2 text-white" style="font-size:12px; cursor:pointer;"/>
                  </div>
                </div>
              </div>
      ...
      <div class="col-lg-3 col-md-3 col-sm-12 col-12 client_list_media">
...
export default {
  ...
  methods:{
    ...
    // <---- For delete adress and check bootle to address  ------->
    async delAddress(i){
      if(this.addressList[i].id>0){
        await this.checkBootleToAddress(this.addressList[i].id, i)
      }
      else{
    ...
    async checkBootleToAddress(id, index){
      ...
      {
        const data = await response.json();
        console.log(data.bottle_count)
        this.modal_info = 'Этот адрес получил бутылка = ' + data.bottle_count;
        ...
        const requestOptions = {
          ...
          try{
            ...
            if(response.status == 201 || response.status == 200)
            {
              ...
              await this.sendTelegramNewClient(data);
    ...
    async sendTelegramNewClient(client){
      let textStr = '/////////  ' + client.fio + '  /////////'
      await this.sendTextToTelegram(textStr);

      for(let i=0; i<client.addresses.length; i++){
        await this.sendLocationToTelegram(client.addresses[i].latidu, client.addresses[i].longitu);
        if(client.addresses[i].address != '' || client.addresses[i].address != null){
          await this.sendTextToTelegram(client.addresses[i].address);
        }
      }
      for(let i=0; i<client.phone_numbers_list.length; i++){
        await this.sendTextToTelegram(client.phone_numbers_list[i].phone_number.slice(4,13))
      }
      // await this.sendTextToTelegram(phoneNumberStr);
    ...
    async sendTextToTelegram(text){
      try{
        ...
        if(res.status == 200 || res.status == 201){
          ...
          console.log('textli apai ishlamadi')
          console.log(data)
          this.$refs.message.error(data)
        }
      }
      catch{
    ...
    async sendPhoneToTelegram(num, name){
      try{
        this.loading = true;
        const res = await fetch('https://api.telegram.org/bot5168670876:AAGp9WQY4jDrk2j_ZU57Cy3-0g6av0TE-ZA/sendContact?chat_id=@extreme_water_clients&phone_number=' + num +'&first_name='+ name + '&last_name=:)');
        this.loading = false;
        if(res.status == 200 || res.status == 201){
          const data = await res.json();
          console.log('textli apai ishladi')
    ...
    async sendLocationToTelegram(lat, long){
      try{
        this.loading = true;
        const res = await fetch('https://api.telegram.org/bot5168670876:AAGp9WQY4jDrk2j_ZU57Cy3-0g6av0TE-ZA/sendLocation?chat_id=@extreme_water_clients&&latitude='+ lat +'&longitude='+ long);
        // const data = await res.json();
        this.loading = false;
        if(res.status == 200 || res.status == 201){
        }
