<template>
  <div class="zaxira">
    ...
    <div v-else class="getProduct mx-3 my-4 ">
      ...
      <div class="davernistTable" v-if="davernost_list.length>0">
        <table class="myTablezaxiradanAlmashtrish mt-4">
            ...
            <tbody>
              <tr v-for="(row,indexs) in davernost_list" :key="indexs" @click="getdavernostClient(row)">
                ...
                <td>
                  <i class="fas fa-trash delIcon mask waves-effect m-0 pl-2"  @click="deleteRow(row)" ></i>
                </td>
      ...
      <div class="mainWorkChange">
        <div class="row px-4">
          <div class="col-3 mt-4  px-3 main_hover_info"  v-for="(item,i) in OstatkaList" :key="i" 
        ...
        <div class="row px-4 pb-3 bg-warning mt-3" v-if="showObyom">
          ...
          <div class="col-2 mt-4  px-3" v-for="(item,index) in changeProduct" :key="index">
            <div class="card pt-2 pr-3 " style="position:relative;" :class="{'bg_red_color': item.auth_user_updator_id == 1}">
              <div class="product_name_price text-right ">
                ...
                <h4 class="mt-2">{{item.qty}} <small>{{item.measure}}</small></h4>
              </div>
              <div class="clickItem border-top mt-2 mx-2">
                <div class="mt-2">
                  <div style="position: relative;">
                    <mdb-input class="m-0 p-0" v-model="item.real_qty" size="md" @input="changeRealQty(index)"  outline  group type="text" validate error="wrong" success="right"/>
          ...
          <div class="col-2  mt-4 px-3">
            <div class=" card pt-2 pr-3" style="position:relative;">
              <div class="product_name_price text-right">
                ...
                <h4 class="mt-2">{{summ_buy}} <small>сум</small></h4>
              </div>
              <div class="clickItem border-top mx-2">
                <div class="mt-2">
                  <div style="position: relative;">
                    <mdb-input class="m-0 p-0" v-model="product_buy" size="md" @input="changeSumma()"  outline  group type="text" validate error="wrong" success="right"/>
                    <small style="position:absolute; top:-7px; left:10px; font-size: 11px;" class="bg-white px-2 py-0">{{$t('kg_ves')}}</small>
                  </div>
                  <div style="position: relative;">
      ...
      <modal-train  :show="invoice_show_input" headerbackColor="white"  titlecolor="black" :title="$t('show_davernis')" 
        @close="invoice_show_input = false" width="85%">
          <template v-slot:body>
            <info-invoice-get @close="invoice_show_input = false" v-if="invoice_show_input"  :option="invoice_list" >
            </info-invoice-get>
          </template>
      </modal-train>
...
data(){
  ...
  components: {
    ...
    checkzaxira, infoInvoiceGet, infoClient,
  ...
  methods: {
    ...
    ...mapMutations(['zaxiraCheckList', 'get_ostatka_check']),
