 ...
 <div class="mainShowGroup">
    ...
    <div v-else class="groupList">
      <div class="row  py-4 backColor px-2 border mx-2 shadow rounded mt-4" v-for="(item,indexRow) in invoice_list" :key="indexRow"
          :class="{'bg_danger': item.check_qty > item.qty+1 || item.check_qty < item.qty-1}">
      ...
      <mdb-row class="mt-1 mx-2">
        <mdb-col col="12">
          <div class="mt-1 mr-3 text-right">
            <mdb-btn color="primary" v-show="invoice_list.length == 0"  @click="delGrroupEmpty" style="font-size: 10.5px"
              p="r4 l4 t2 b2">
      ...
      for(let i=0; i<data.length;i++){
        ...
        try{
          ...
          if(resp.status == 201 || resp.status == 200){
            console.log('this is by id 88855');
            console.log(data);
            info.summ = data.summ;
