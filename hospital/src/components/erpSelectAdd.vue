    ...
    <div class="card options" v-if="areOptionsVisible">
      <div class="down_list">
        <div class="container">
          ...
          <mdb-btn class="top-btn py-1 px-3 mt-3" v-if="btnadd==true && search != ''" color="primary"
  ...
  methods: {
    ...
    changeRemove(){
      for(let i = 0; i <this.options.length; i++){
        if(this.search == this.options[i].name_1){
          this.munp = false;
            ...
            filteredList: function(){
                if(this.search)
                {
                  return this.options.filter((item)=>{
                    return this.search.toLowerCase().split(' ').every(v => item.name_1.toLowerCase().includes(v))
                  })
                }else
                {
                  return this.options;
