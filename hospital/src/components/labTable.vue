      ...
      <table  class="myTableLab">
                ...
                <tr v-for="(row,rowIndex) in datasource.rows" :key="rowIndex">
                    ...
                    <td v-for="(column,i) in datasource.columns" :key="i">
                        <span v-show="row[column] !== true && row[column] !== false && column !== 'create_date' && column !== 'colorCode'">{{row[column]}}</span>
                        <span v-show="column == 'create_date'" style="font-weight: 450; ">{{row[column].slice(0,10)}} <span class="ml-4 text-primary">{{row[column].slice(11,16)}}</span></span>
                    </td>
    ...
    async checklist (option){
      ...
      clicked_filter()
      {
        ...
        showRow(ev)
        {
          ...
          console.log(this.datasource.rows[this.num_target])
          this.$router.push('/lab_show/' + this.datasource.rows[this.num_target].Id)
        },
        deleteRow(ev) {
            this.confirm = true
