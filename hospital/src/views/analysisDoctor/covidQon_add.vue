<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Коронавирус кровь (COVID 19)</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Лаборатор таҳлил натижаси (LGG)')"  v-model="result_lgg" outline/>
          <small v-if="$v.result_lgg.$dirty  && !$v.result_lgg.required" class="invalid-text pt-4 pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
        </div>
         <div style="position:relative;">
          <mdb-input :label="('Лаборатор таҳлил натижаси (LGM)')"  v-model="result_lgm" outline/>
          <small v-if="$v.result_lgm.$dirty && !$v.result_lgm.required" class="invalid-text pt-4 pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
        </div>
...
export default {
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalCovidQon/' + this.id);
        const data = await res.json();
        console.log(data)
          this.result_lgm = data.result_lgm
          this.result_lgg = data.result_lgg
          this.id = data.id
  ...
  computed:{
    ...mapGetters(['get_patient_client_list', 'get_client_info']),
  ...
  methods:{
    ...mapActions(['fetch_patient_client', 'fetch_client_info']),
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          body: JSON.stringify({
          "result_lgm": this.result_lgm,
          "result_lgg": this.result_lgg,
          "id": this.id,
          "patientsId":  this.patient_id,
          "device_name": "covid19_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalCovidQon', requestOptions)
        const data = await response.json()
        if(data.id){
          this.cls_wnd()
          this.sendFinish()
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/covid19_print/' + data.id)
        }
        else{
          // this.$refs.msg.error('Error_successfully')
          this.loading = false;
