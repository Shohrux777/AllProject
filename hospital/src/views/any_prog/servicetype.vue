  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">{{$t('type_service')}}</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        ...
        <mdb-modal-body>
          <serviceTypeAdd :options="editData"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import serviceTypeAdd from "../../components/new_prog_add/serviceType_add"
  ...
  export default {
    ...
    methods: {
      ...mapActions(['fetch_service_type']),
      ...mapMutations(['service_type_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/ServiceTypes/" + del_data.id, requestOptions);
          ...
          if(data)
          {
            ...
            this.service_type_row_delete(index);
          }
          else{
