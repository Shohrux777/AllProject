  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">Должность таблица</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('Добавить должность')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <positionAdd :options="editData"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import positionAdd from "../../components/new_prog_add/position_add"
  ...
  export default {
    ...
    async mounted(){
      this.loading=true;
      await this.fetch_position();
      this.loading = false;
    },
    computed: mapGetters(['get_position_list']),
    methods: {
      ...mapActions(['fetch_position']),
      ...mapMutations(['position_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/Positions/" + del_data.id, requestOptions);
          ...
          if(data)
          {
            ...
            this.position_row_delete(index);
          }
          else{
