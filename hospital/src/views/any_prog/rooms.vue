  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">Список комната</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false" size="md" light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('Добавить комната')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <roomsAdd :options="editData"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import roomsAdd from "../../components/new_prog_add/rooms_add"
  ...
  export default {
    ...
    computed: mapGetters(['get_rooms_list']),
    methods: {
      ...mapActions(['fetch_rooms']),
      ...mapMutations(['rooms_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/Rooms/" + del_data.id, requestOptions);
