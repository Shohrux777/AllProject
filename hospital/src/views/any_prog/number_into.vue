  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">Число услуги таблица</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:500;">Добавить Число услуги </mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <numIntoAdd :options="editData"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import numIntoAdd from "../../components/new_prog_add/number_into_add"
  ...
  export default {
    ...
    async mounted(){
      this.loading = true;
      await this.fetch_number_into();
      this.loading = false;
    },
    computed: mapGetters(['get_number_into']),
    methods: {
      ...mapActions(['fetch_number_into']),
      ...mapMutations(['number_into_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          if(data)
          {
            ...
            this.number_into_row_delete(index);
          }
          else{
