    ...
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeightAll"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold;" class="mb-5 headerTitle">Гликированный гемоглобин</h3>
          ...
          <table border class="allWidthLabTable" style="margin-top:35px;">
            ...
            <tbody >
              <tr >
                <td class="fs">1</td>
                <td class="fs">Гликированный гемоглобин </td>
                <td class="fs">{{option.name_1}}</td>
                <td class="fs"><p class="m-0 p-0 pb-1">4.5-7.0 %</p>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_10/getFullInfoById?id=' + this.id)
  ...
  methods: {
    ...
    async send(){
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
        "analiz_name": "Гликированный гемоглобин",
        "analiz_base_str": this.base64,
        "id": 0,
        "patientsId": this.option.patientsId,
        "sended_status":  true,
        "doctor_name": 'glik_gemok_print',
