    ...
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeightAll"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold;"  class="mb-5 headerTitle">ВСК по Сухареву</h3>
          ...
          <table border class="allWidthLabTable" style="margin-top:35px;">
            ...
            <tbody >
              <tr >
                <td class="fs">1</td>
                <td class="fs">ВСК по Сухареву </td>
                <td class="fs">{{option.name_1}}</td>
                <td class="fs"><p class="m-0 p-0 pb-1">начало - 30 сек - 2 мин</p>
                  <p class="m-0 p-0 pb-1">конец - 3 - 5 мин</p>
                </td>
              </tr>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_8/getFullInfoById?id=' + this.id)
