    ...
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeightAll"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold;"  class="mb-5 headerTitle">Спид (Вич) екпресс</h3>
          ...
          <table border class="allWidthLabTable" style="margin-top:35px;">
            ...
            <tbody>
              <tr v-show="option.name_1 != null && option.name_1 != ''">
                <td class="fs">1</td>
                <td class="fs">Спид (Вич) екпресс</td>
                <td class="fs">{{option.name_1}}</td>
                <td class="fs">отрицательный
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_11/getFullInfoById?id=' + this.id)
