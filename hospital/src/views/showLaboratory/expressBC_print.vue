    ...
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeight"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold; font-size:55px;" class="mb-5">ЭКСПРЕСС ТЕСТ</h3>
          ...
          <table border class="allWidthLabTable"  style="margin-top:35px;">
            <thead >
              <tr >
                ...
                <th class="fs" ><span class="font-weight-bold py-2">Норма</span></th>
              </tr>
            </thead>
            <tbody class="fs">
              <tr>
                <td class="fs">Гепатит В НBsAg</td>
                <td class="fs" >{{option.gepatit_b}}</td>
                <td class="fs"> Отрицательно</td>
              </tr>
              <tr>
                <td class="fs">Гепатит C Anti-HCV</td>
                <td class="fs" >{{option.gepatit_c}}</td>
                <td class="fs"> Отрицательно</td>
              </tr>
            </tbody>
          </table>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalExpressGepatitBC/' + this.id)
