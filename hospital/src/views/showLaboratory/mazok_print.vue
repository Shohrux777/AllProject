  ...
  <div>
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeight"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold; " class="mb-5 headerTitle">Анализ мазка</h3>
          ...
          <table border class="allWidthLabTable" style="margin-top:35px;">
            <thead>
              <tr class="border-bottom">
                <th  class="fs font-weight-bold">Показатель</th>
                <th  class="fs font-weight-bold">Вагина</th>
                <th  class="fs font-weight-bold py-2">Сервикс</th>
                <th  class="fs font-weight-bold py-2">Уретра</th>
              </tr>
            </thead>
            <tbody >
              <tr class="border-bottom">
                <td class="fs">Лейкоциты</td>
                <td class="fs">{{option.leykositi_vagina}}</td>
                <td class="fs">{{option.leykositi_serviks}}</td>
                <td class="fs">{{option.leykositi_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Плоский эпителий</td>
                <td class="fs">{{option.ploskiy_epiteliy_vagina}}</td>
                <td class="fs">{{option.ploskiy_epiteliy_serviks}}</td>
                <td class="fs">{{option.ploskiy_epiteliy_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Гонококки</td>
                <td class="fs">{{option.gonokokki_vagina}}</td>
                <td class="fs">{{option.gonokokki_serviks}}</td>
                <td class="fs">{{option.gonokokki_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Трихомонады</td>
                <td class="fs">{{option.trixomonadi_vagina}}</td>
                <td class="fs">{{option.trixomonadi_serviks}}</td>
                <td class="fs">{{option.trixomonadi_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Ключевые клетки</td>
                <td class="fs">{{option.kluchoviy_kletki_vagina}}</td>
                <td class="fs">{{option.kluchoviy_kletki_serviks}}</td>
                <td class="fs">{{option.kluchoviy_kletki_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Дрожжи</td>
                <td class="fs">{{option.drojji_vagina}}</td>
                <td class="fs">{{option.drojji_serviks}}</td>
                <td class="fs">{{option.drojji_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Микрофлора</td>
                <td class="fs">{{option.mikroflora_vagina}}</td>
                <td class="fs">{{option.mikroflora_serviks}}</td>
                <td class="fs">{{option.mikroflora_uretra}}</td>
              </tr>
              <tr>
                <td class="fs">Слизь</td>
                <td class="fs">{{option.sliz_vagina}}</td>
                <td class="fs">{{option.sliz_serviks}}</td>
                <td class="fs">{{option.sliz_uretra}}</td>
              </tr>
    ...
    <div class="d-flex justify-content-center">
      <div style="width:83%; text-align:right; position:fixed; bottom:10px; right:108px;" v-if="font_size == false">
        <mdb-btn color="success px-4 py-1" v-if="false" style="font-size:15px;" type="submit" @click="printed"><mdb-icon icon="print" /></mdb-btn>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalMazok/' + this.id)
