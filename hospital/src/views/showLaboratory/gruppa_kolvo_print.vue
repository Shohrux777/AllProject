    ...
    <div class="rv_lab allWidthLab" id="table" style="position:relative;">
      ...
      <div class="lab_answer upHeightAll"  >
        <div class=" " style="display:flex; justify-content:center; flex-direction:column;">
          <h3 style="width:100%; text-align:center; font-weight:bold;"  class="mb-5 headerTitle">Резус-фактор ва қон группаси таҳлили</h3>
          ...
          <table border class="allWidthLabTable" style="margin-top:35px;">
            ...
            <tbody >
              ...
              <tr v-show="option.name_3 != null && option.name_3 != ''">
                <td class="fs">3</td>
                <td class="fs">Коллоид синамаси </td>
...
export default {
 async mounted() {
  ...
  async created(){
    if(this.id>0){
      this.loading =true;
      const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_17/getFullInfoById?id=' + this.id)
  ...
  methods: {
    ...
    async send(){
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
        "analiz_name": "Резус-фактор ва қон группаси таҳлили",
        "analiz_base_str": this.base64,
        "id": 0,
        "patientsId": this.option.patientsId,
        "sended_status":  true,
        "doctor_name": 'gruppa_kolvo_print',
