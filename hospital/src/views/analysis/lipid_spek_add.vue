<template>
  <div class="name_23ya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Липидный спектр</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Общий холестерин  ')"  v-model="name_1" outline/>
          <small class="pos_absolute">
            {{simbol}}=5,2 ммоль/л
          </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('ЛПВП-холестерин')"  v-model="name_2" outline/>
          <small class="pos_absolute">
            жен - 1.15 ммоль/л
            муж - >0.9 ммоль/л
          </small>
      ...
      <div class="col-6" >
        ...
        <div style="position:relative;">
          <mdb-input :label="('Триглицериды')"  v-model="name_4" outline/>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_9/getFullInfoById?id=' + this.id);
    ...
    async send(){
      ...
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "lipid_spek_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_9', requestOptions)
        const data = await response.json()
          console.log(data)
        ...
        if(data.id){
          ...
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/lipid_spek_print/' + data.id)
