    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Фибриноген ')"  v-model="name_1" outline/>
          <small class="pos_absolute">M: 200-400 мг/дл
            берем I-II трим 240 - 500 мг/дл</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Активное частичное тромбопластиновое время')"  v-model="name_2" outline/>
          <small class="pos_absolute">24-42 сек</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('МНО')"  v-model="name_3" outline/>
          <!-- <small v-if="$v.name_3.$dirty && !$v.name_3.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">0,8-1,3</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('ПТИ')"  v-model="name_4" outline/>
          <!-- <small v-if="$v.name_4.$dirty && !$v.name_4.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">80-107%</small>
        </div>
      ...
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Тромбиновое время')"  v-model="name_5" outline/>
          <!-- <small v-if="$v.name_5.$dirty  && !$v.name_5.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">{{tnd}} 30 сек</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Тромбиновое время (соотношение)')"  v-model="name_6" outline/>
          <!-- <small v-if="$v.name_6.$dirty  && !$v.name_6.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">0,8-1,5</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('АЧТВ (соотношение)')"  v-model="name_7" outline/>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_3/getFullInfoById?id=' + this.id);
    ...
    async send(){
      ...
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "kolagram_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name":  localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_3', requestOptions)
        const data = await response.json()
          console.log(data)
        ...
        if(data.id){
          ...
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/kolagram_print/' + data.id)
