<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Экспресс Гепатит тест</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Гепатит В НBsAg')"  v-model="gepatit_b" outline/>
          <small v-if="$v.gepatit_b.$dirty && !$v.gepatit_b.required" class="invalid-text pt-4 pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
        </div>
         <div style="position:relative;">
          <mdb-input :label="('Гепатит C Anti-HCV')"  v-model="gepatit_c" outline/>
          <small v-if="$v.gepatit_c.$dirty && !$v.gepatit_c.required" class="invalid-text pt-4 pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
        </div>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalExpressGepatitBC/' + this.id);
        const data = await res.json();
        console.log(data)
          this.gepatit_c = data.gepatit_c,
          this.gepatit_b = data.gepatit_b,
          this.id = data.id
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          body: JSON.stringify({
          "gepatit_c": this.gepatit_c,
          "gepatit_b": this.gepatit_b,
          "id": this.id,
          "patientsId":  this.patient_id,
          "device_name": "expressBC_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalExpressGepatitBC', requestOptions)
        const data = await response.json()
        if(data.id){
          this.cls_wnd()
          this.sendFinish()
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/expressBC_print/' + data.id)
