    ...
    <div v-else class="mx-1 row">
      <div class="col-4" >
        ...
        <div style="position:relative;">
          ...
          <small class="pos_absolute">Э: 44 – 115
            А: 44 – 97
            </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Билирубин: Умумий ')"  v-model="name_8" outline/>
          <!-- <small v-if="$v.name_8.$dirty  && !$v.name_8.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">3,4 – 20,5</small>
        </div>
        
      </div>
      <div class="col-4" >
        <div style="position:relative;">
          <mdb-input :label="('Билирубин: Боғланган')"  v-model="name_9" outline/>
          <!-- <small v-if="$v.name_9.$dirty  && !$v.name_9.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">0,86 – 5,3</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Билирубин: Эркин ')"  v-model="name_10" outline/>
          <!-- <small v-if="$v.name_10.$dirty && !$v.name_10.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">1,7 – 17,1</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Аланинаминотрансфераза (АЛТ)')"  v-model="name_11" outline/>
          <!-- <small v-if="$v.name_11.$dirty && !$v.name_11.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">{{simbol}} 40</small>
        </div>


        
        <div style="position:relative;">
          <mdb-input :label="('Аспартатаминотрансфераза (АСТ)')"  v-model="name_12" outline/>
          <!-- <small v-if="$v.name_12.$dirty  && !$v.name_12.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">{{simbol}} 35
          </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Лактатдегидрогеназа (ЛДГ)')"  v-model="name_13" outline/>
      ...
      <div class="col-4" >
        ...
        <div style="position:relative;">
          <mdb-input :label="('Глюкоза  (тук коринга)')"  v-model="name_24" outline/>
          <!-- <small v-if="$v.name_5.$dirty  && !$v.name_5.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">3,2 – 6,1</small>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_6/' + this.id);
    ...
    async send(){
      ...
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "biox_krov_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_6', requestOptions)
        const data = await response.json()
          console.log(data)
        ...
        if(data.id){
          ...
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/biox_krov_print/' + data.id)
        }
        else{
          // this.$refs.msg.error('Error_successfully')
          this.loading = false;
