<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Ревмопробы</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('С-реактивний белок (СРБ)')"  v-model="srb" outline/>
          <small v-if="$v.srb.$dirty  && !$v.srb.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
          <!-- <small class="pos_absolute">6 мг/л</small> -->
          <small class="pos_absolute">0-20 IU/mL</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Ревматоидный фактор (РФ)')"  v-model="rf" outline/>
          <small v-if="$v.rf.$dirty   && !$v.rf.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
          <small class="pos_absolute">0-6  IU/mL</small>
          <!-- <small class="pos_absolute">12 ед/л </small> -->
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Антистрептолизин-О (асло)')"  v-model="aslo" outline/>
          <small v-if="$v.aslo.$dirty && !$v.aslo.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
          <small class="pos_absolute">0-200 IU/mL </small>
          <!-- <small class="pos_absolute">200 ед/л </small> -->
        </div>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalRemaproba/' + this.id);
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "remaproba_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalRemaproba', requestOptions)
        const data = await response.json()
        if(data.id){
          this.cls_wnd();
          this.sendFinish();
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/remaproba_print/' + data.id)
