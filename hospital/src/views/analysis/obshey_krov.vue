<template>
  <div class="bg-white"  style="min-height: 100vh; border-radius:3px;">
    <div class="border-bottom d-flex justify-content-between">
      <router-link to="/obshey_krov">
         <h5 class="m-0 ml-4 d-flex" style="padding: 16px 0px">
            ОБЩИЙ АНАЛИЗ КРОВИ</h5>
      </router-link>
      <div>
         <router-link to="/obshey_krov_add/0">
...
export default {
  ...
  methods: {
    ...
    async checklist (){
      ...
      for_edit(edit_data)
      {
        this.$router.push("/obshey_krov_add/" + edit_data.id);
      },
      async for_delete(del_data)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalAnaliz_4/" + del_data.id, requestOptions);
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_4/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
