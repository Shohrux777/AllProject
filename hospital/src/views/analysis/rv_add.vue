<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Иммуноферментный анализ крови « АНТИ-ЛЮИС-GМ » </h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Антитела  классов GМ  к  Treponema pallidum  RW-СИФИЛИС')"  v-model="syplhilis_rpr" outline/>
          <small v-if="$v.syplhilis_rpr.$dirty && !$v.syplhilis_rpr.required" class="invalid-text pt-4 pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
        </div>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalRv/' + this.id);
        const data = await res.json();
        console.log(data)
        this.syplhilis_rpr= data.syplhilis_rpr;
          this.id = data.id
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          body: JSON.stringify({
          "syplhilis_rpr": this.syplhilis_rpr,
          "id": this.id,
          "patientsId":  this.patient_id,
          "device_name": "rv_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalRv', requestOptions)
        const data = await response.json()
        if(data.id){
          this.cls_wnd();
          this.sendFinish();
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/rv_print/' + data.id)
        }
        else{
          // this.$refs.msg.error('Error_successfully')
          this.loading = false;
