<template>
  <div class="name_23ya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Иммуноферментный анализ крови на гормоны</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
   ...
   <loader v-if="loading"/>
    <div v-else class="mx-1 row">
      ...
      <div class="col-6" >
        ...
        <div style="position:relative;">
          <mdb-input :label="('Лютеинизирующий гормон(ЛГ)')"  v-model="name_6" outline/>
          <!-- <small v-if="$v.name_6.$dirty  && !$v.name_6.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">Дети до 11 лет: 0,1-5 mIU/mL
            Мужчины: 1.1-25 mIU/mL
          </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Пролактин')"  v-model="name_7" outline/>
          <!-- <small v-if="$v.name_7.$dirty  && !$v.name_7.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">1,3 - 18,1 мг-мл
          </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Тестостерон')"  v-model="name_8" outline/>
          ...
          <small class="pos_absolute">
            Женщины: {{simbol}} 1.0 ng/mL
            Мужчины: 2.2-10.5 ng/mL
          </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Тироид - Анти - Тпо')"  v-model="name_9" outline/>
          <!-- <small v-if="$v.name_8.$dirty  && !$v.name_8.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">
            Мужчины: 0-30 0-0 МЕ/мл
            Женщины: 0-30 0-0 МЕ/мл
            Женщины старше:  лет 0-50 0-0 МЕ/мл
          </small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Инсулин')"  v-model="name_10" outline/>
          <!-- <small v-if="$v.name_8.$dirty  && !$v.name_8.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small> -->
          <small class="pos_absolute">
            2.0 - 29.0 0-0 мкМЕ/мл
          </small>
        </div>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_5/' + this.id);
    ...
    async send(){
      ...
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "qon_garmon_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_5', requestOptions)
        const data = await response.json()
          console.log(data)
        ...
        if(data.id){
          ...
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/qon_garmon_print/' + data.id)
        }
        else{
          // this.$refs.msg.error('Error_successfully')
          this.loading = false;
