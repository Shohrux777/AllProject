<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Анализ витамин – Д</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Витамин – Д ')"  v-model="name_32" outline/>
          <small v-if="$v.name_32.$dirty && !$v.name_32.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
          <small class="pos_absolute">Низкий :  <span>{{tag}}</span> 20 нг/мл Недостаточный :  20-30 нг/мл  Достаточный :      > 30 нг/мл </small> 
        </div>
      </div>
    </div>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_1/getFullInfoById?id=' + this.id);
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "vitD_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_1', requestOptions)
        const data = await response.json()
        if(data.id){
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.cls_wnd()
          this.$router.push('/vitD_print/' + data.id)
