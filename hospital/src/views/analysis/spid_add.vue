<template>
  <div class="name_23ya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Спид (Вич) екпресс</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row mb-4">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('Спид (Вич) екпресс')"  v-model="name_1" outline/>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_11/getFullInfoById?id=' + this.id);
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "spid_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_11', requestOptions)
        const data = await response.json()
          console.log(data)
        ...
        if(data.id){
          ...
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/spid_print/' + data.id)
