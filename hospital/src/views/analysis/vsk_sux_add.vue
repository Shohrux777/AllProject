<template>
  <div class="name_23ya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">ВСК по Сухареву</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row mb-4">
      <div class="col-6" >
        <div style="position:relative;">
          <mdb-input :label="('ВСК по Сухареву ')"  v-model="name_1" outline/>
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_8/getFullInfoById?id=' + this.id);
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          "device_name": "vsk_sux_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalAnaliz_8', requestOptions)
        const data = await response.json()
          console.log(data)
        ...
        if(data.id){
          ...
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/vsk_sux_print/' + data.id)
