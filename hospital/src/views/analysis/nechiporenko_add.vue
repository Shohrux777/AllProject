<template>
  <div class="bakteriya bg-white" style="min-height: 100vh;">
    <div class="header text-center border-bottom py-2">
      <h4 class="text-primary">Анализ мочи по нечипоренко</h4>
    </div>
    <div class="row mt-2">
      <div class="col-4">
    ...
    <div v-else class="mx-1 row">
      <div class="col-6" >
        <div style="position:relative;">
          ...
          <!-- <small class="pos_absolute">2000  / в 1 мл</small> -->
          <small class="pos_absolute">До 2000</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Эритроциты')"  v-model="eritrositi" outline/>
          ...
          <!-- <small class="pos_absolute">1000   / в 1 мл</small> -->
          <small class="pos_absolute">До 1000</small>
        </div>
        <div style="position:relative;">
          <mdb-input :label="('Цилиндры')"  v-model="silindri" outline/>
          <small v-if="$v.silindri.$dirty && !$v.silindri.required" class="invalid-text  pl-4" style="position:absolute; bottom:-18px; left:-15px;"> 
            {{$t('select_item')}}
          </small>
          <!-- <small class="pos_absolute">20 / в 1 мл</small> -->
  ...
  async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/HospitalNechiporenko/' + this.id);
        const data = await res.json();
        console.log(data)
          this.eritrositi= data.eritrositi;
          this.leykositi= data.leykositi;
          this.silindri= data.silindri;
          this.id = data.id
    ...
    async send(){
      try{
        ...
        const requestOptions = {
          ...
          "silindri": this.silindri,
          "id": this.id,
          "patientsId":  this.patient_id,
          "device_name": "nechiporenko_print",
          "doctor_name": this.doctor_name,
          "creator_doctor_name": localStorage.docName
          })
        };
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + '/HospitalNechiporenko', requestOptions)
        const data = await response.json()
        if(data.id){
          this.cls_wnd()
          this.sendFinish();
          this.loading = false;
          this.$refs.message.success('Added_successfully')
          this.$router.push('/nechiporenko_print/' + data.id)
        }
        else{
          // this.$refs.msg.error('Error_successfully')
          this.loading = false;
