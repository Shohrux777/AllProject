...
export default {
  ...
  methods: {
    ...
    async checklist (){
      ...
      for_edit(edit_data)
      {
        this.$router.push("/spid_add/" + edit_data.id);
      },
      async for_delete(del_data)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalAnaliz_11/" + del_data.id, requestOptions);
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/HospitalAnaliz_11/getPagination?page=' + this.get_pagination.page + '&size=' + this.get_pagination.size);
