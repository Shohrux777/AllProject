  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">{{$t('bronRoomAddBeds')}}</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('bronRoomAddBeds')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <bronRoomAddBeds_add :options="editData" />
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import bronRoomAddBeds_add from "../../views/bron/bronRoomAddBeds_add"
  ...
  export default {
    ...
    async mounted(){
      this.loading = true;
      await this.fetch_bron_rb();
      this.loading = false;
    },
    computed: mapGetters(['bron_rb_list']),
    methods: {
      ...mapActions(['fetch_bron_rb']),
      ...mapMutations(['bron_rb_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalBronRoomAddBeds/" + del_data.id, requestOptions);
          const data = await response.json();
          if(data)
          {
            // this.alert_text = localizeFilter('Successfully_removed');
            this.$refs.message.error('Successfully_removed')
            this.bron_rb_row_delete(index);
          }
          else{
