...
<script>
  ...
  export default {
    ...
    computed: mapGetters(['bron_beds_list']),
    methods: {
      ...mapActions(['fetch_bron_beds']),
      ...mapMutations(['bron_beds_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalBronRoomBeds/" + del_data.id, requestOptions);
          const data = await response.json();
          if(data)
          {
            // this.alert_text = localizeFilter('Successfully_removed');
            this.$refs.message.error('Successfully_removed')
            this.bron_beds_row_delete(index);
          }
          else{
