  ...
  export default {
    ...
    methods: {
      ...
      async func_vozvrat(id){
        const res = await fetch(this.$store.state.hostname + '/HospitalBronRoomPayments/addPaymentVozvrat?payment_id=' + id + '&vozvrat_sum=' + this.vozvrat_sum + '&days=' + this.vozvrat_day);
        const res_data = await res.json();
        console.log('res_data')
        console.log(res_data)
        if(res.status == 200 || res.status == 201){
