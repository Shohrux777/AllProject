          ...
          <table class="myTable">
            ...
            <tbody>
              <tr v-for="(row,rowIndex) in bonus_contragent_list" :key="rowIndex">
                ...
                <td> <span >{{row.qtySumm}}</span> </td>
                <td> <span >{{row.realQty}}</span> </td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async get_get_(){ 
        const response = await fetch(this.$store.state.hostname + '/ContragentAdditionalPaymentBefohandFullInfoes')
        const data = await response.json()
        this.bonus_contragent_list = data
        this.summa = 0;
        for(var i = 0; i < data.length; i++) {
          this.summa += data[i].realQty
        }
        console.log(data)
      },
      add(){
