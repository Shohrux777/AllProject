      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">Добавить группу услуг не показывать врача</h5>
          </div>
          <div class="plus">
        ...
        <loader v-if="loading"/>
        <anyTable v-else
          :datasource="get_service_group_show_doc"
          @for_edit="for_edit"
...
<script>
  ...
  export default {
    ...
    async mounted(){
      this.loading = true;
      await this.fetch_service_group_show_doc();
      this.loading = false;
    },
    computed: mapGetters(['get_service_group_show_doc']),
    methods: {
      ...mapActions(['fetch_service_group_show_doc']),
      ...mapMutations(['service_group_show_doc_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalServiceTypeByGroupPermissions/" + del_data.id, requestOptions);
          ...
          if(data)
          {
            ...
            this.service_group_show_doc_row_delete(index);
