      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">{{$t('package_service')}}</h5>
          </div>
          <div class="plus">
            <router-link to="/package_add/0">
...
export default {
  components: {
   ...
   data() {
    ...
    computed: mapGetters(['get_package_list']),
    methods:
    {
      ...mapActions(['fetch_m_package']),
      ...mapMutations(['package_delete_row']),
      for_edit(edit_data)
      {
        this.$router.push("/package_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/HospitalServiceTypePacks/" + del_data.id, requestOptions);
      ...
      // async update_column()
      // {
      //   const res = await fetch(this.$store.state.hostname + '/package_buy_column/' + this.user_id);
