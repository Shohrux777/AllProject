              ...
              <div class="price_all_item card">
                <div class="qty borderSolder py-2">
                    ...
                    <div class="text-right px-3 mt-1">
                      <p>{{cashfull.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}} сум</p>
                    </div>
                  </div>
              </div>
            </div>
            ...
            <div class="col-3 pr-0">
              <div class="price_all_item card">
                <div class="qty borderSolder py-2">
                    ...
                    <div class="text-right px-3 mt-1">
                      <p>{{rasxod_money.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}} сум</p>
                    </div>
                  </div>
              </div>
            </div>
            ...
            <tbody>
              ...
              :class="{ 'text-danger': row.hospitalServiceTypeGroupName == 'RETURNED',
               ...
               'text-secondary': row.hospitalServiceTypeGroupName == 'VOZVRAT' }">
                ...
                <td> <span >{{row.cashSumm.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</span> </td>
                <td> <span >{{row.cardSumm.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</span> </td>
                <td> <span >{{row.count}}</span> </td>
                <td> <span >{{(row.cashSumm + row.cardSumm).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</span> </td>
              ...
              <tr >
                ...
                <td> <span  class="text-primary">{{cash.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}} сум </span></td>
              </tr>
               <tr >
                <td> <span class="text-success"></span>{{$t('card')}} </td>
                <td> <span  class="text-primary">{{card.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}} сум </span></td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          for(let i=0; i<data.length; i++){
            if(data[i].hospitalServiceTypeGroupName != 'VOZVRAT' && data[i].hospitalServiceTypeGroupName != 'REJECTED'){
              if(data[i].hospitalServiceTypeGroupName == 'RETURNED'){
                ...
                this.rasxod_money += data[i].cashSumm
                this.rasxod_money += data[i].cardSumm
                // this.qtys -= data[i].count
              }
              else{
                this.cash += data[i].cashSumm
                this.cashfull += data[i].cashSumm
                this.card += data[i].cardSumm
                this.qtys += data[i].count
              }
            }
