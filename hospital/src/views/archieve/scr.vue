  ...
  <div>
      ...
      <div class="bg-white p-4 mb-5 pt-4 shadow" style="border-radius:5px; position:relative;">
        <div style="height: 60px;" class="d-flex justify-content-between border-bottom align-items-center  ">
          <div class="title d-flex align-items-center">
            <h5 class="m-0 ">Датчик и Цветь таблица</h5>
          </div>
          <div class="plus">
    ...
    <!-- <div :class="{'showing':show}">
      ...
      <mdb-modal  :show="show"  @close="show = false"  light>
        <mdb-modal-header>
          <mdb-modal-title style="font-weight:  500;">{{$t('Добавить Датчик и Цветь')}}</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <scrAdd :options="editData"/>
        </mdb-modal-body>
      </mdb-modal>
...
<script>
  import scrAdd from "../../components/archieve_add/scr_add"
  ...
  export default {
    ...
    computed: mapGetters(['get_scr_add_list']),
    methods: {
      ...mapActions(['fetch_scr_add']),
      ...mapMutations(['scr_add_row_delete']),
      ...
      async for_delete(del_data,index){
          ...
          const response = await fetch(this.$store.state.hostname + "/DatchikItems/" + del_data.id, requestOptions);
