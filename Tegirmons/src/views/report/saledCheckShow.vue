    ...
    <div  id="printForm"  class="ckeck_main_sell  border bg-white"  style="width:400px; height: 100%;">
      ...
      <div class="infoHeader" style="width:100%;">
        <div style="margin: 5px 0px;">
          <div  style=" display:flex; flex-wrap: wrap; justify-content: space-between;  margin-top:3px;">
            ...
            <span style="font-size: 11.5px;" class="paymentCheckPrintCheck" v-if="option.tegirmonAuthid">{{option.auth.user.fio}}</span>
          </div>
        </div>
        <div style="margin: 5px 0px;">
          <div  style=" display:flex; flex-wrap: wrap; justify-content: space-between;  margin-top:3px;">
            <span style="font-size:11px;" class="paymentCheckPrintCheck">Напечатан </span>
            <span style="font-size: 11.5px;" class="paymentCheckPrintCheck">{{option.created_date_time.slice(0,10)}} <span class="ml-1">{{option.created_date_time.slice(11,16)}}</span></span>
          </div>
        </div>
      </div>
      <div class="infoList">
        <table style=" width:100%;">
          ...
          <tbody>
            <tr v-for="(item,i) in option.payments" :key="i" style="border-bottom: 0.5px solid #CED4DA;">
              <td class="paymentCheckPrintCheck" style="text-align: left; font-size:11px;">{{item.product.name}}</td>
              <td class="paymentCheckPrintCheck" style="text-align: center; font-size:11px;">{{item.qty}}</td>
              <td class="paymentCheckPrintCheck" style="text-align: center; font-size:11px;">{{item.product.price}}</td>
              <td class="paymentCheckPrintCheck" style="text-align: right; font-size:11px;">{{item.summa}}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div style="margin: 5px 0px;">
        <div  style=" display:flex; flex-wrap: wrap; justify-content: space-between;  margin-top:3px;">
          ...
          <span style="font-size: 10.5px; font-weight: bold;" class="paymentCheckPrintCheck ">{{option.summ}}</span>
        </div>
      </div>
      <div style="border-bottom: 1px dashed black;">
        <!-- ******************************************************************************************** -->
