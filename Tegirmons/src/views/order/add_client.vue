    ...
    <div class="col-8">
      <div class="add_davernis p-4">
        ...
        <mdb-row class="mt-3">
          ...
          <mdb-col col="8">
            <input class="m-0 form-control" style="height:34px;" v-mask="'(##) ###-##-##'" v-model="phone_number" placeholder="(##) ###-##-##"
              @input="SearchClientNamePass(phone_number)"
        ...
        <mdb-row class="mt-3">
          ...
          <mdb-col col="8">
            <input class="m-0 form-control" style="height:34px;" v-mask="'(##) ###-##-##'" v-model="phone_number1" placeholder="(##) ###-##-##"
              @input="SearchClientNamePass(phone_number1)"
             outline  group type="text" validate error="wrong" success="right"/>
        ...
        <mdb-row class="mt-3">
          ...
          <mdb-col col="8">
            <input class="m-0 form-control" style="height:34px;" v-mask="'(##) ###-##-##'" v-model="phone_number2" placeholder="(##) ###-##-##"
              @input="SearchClientNamePass(phone_number2)"
             outline  group type="text" validate error="wrong" success="right"/>
        ...
        <mdb-row class="mt-3">
          ...
          <mdb-col col="8">
            <input class="m-0 form-control" style="height:34px;" v-model="passport_number" v-mask="'AA#######'" placeholder="AA#######"
            @input="SearchClientNamePass(passport_number)" size="md" outline  group type="text" validate error="wrong" success="right"/>
...
export default {
  ...
  methods: {
    ...
    async getBemorId(i, data){
      ...
      this.phone_number1 = data.phone_number1;
    ...
    save_data :  async function(){
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/TegirmonOrderClient", requestOptions);
        this.loading = false;
        if(response.status == 201 || response.status == 200)
        {
        // console.log('dasdas')
    ...
    async SearchClientNamePass(search){
      ...
      try{
        // this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/TegirmonOrderClient/getPaginationSearchByFioOrPassportSerailNumberOrHomeOrMobilePhoneNumber?page=0&size=100&fio_or_serial_number=" + search);
        const data = await response.json();
        // console.log(data)
        // this.loading = false;
        if(response.status == 201 || response.status == 200)
    ...
    async SearchClientNote(search){
      ...
      try{
        // this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/TegirmonOrderClient/getPaginationByNote?page=0&size=100&note_str=" + search);
        const data = await response.json();
        // this.loading = false;
        if(response.status == 201 || response.status == 200)
        {
    ...
    async SearchClientBornDate(search){
      ...
      try{
        // this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/TegirmonOrderClient/getPaginationByBornDate?page=0&size=100&born_date_str=" + search);
        const data = await response.json();
        // this.loading = false;
        if(response.status == 201 || response.status == 200)
        {
