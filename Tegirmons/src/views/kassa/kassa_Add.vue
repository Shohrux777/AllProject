        ...
        <mdb-row>
          ...
          <mdb-col col="5">
            ...
            <small
              ...
              v-else
            >
              Kassani nomini kiriting. 
            </small>
          </mdb-col>
        </mdb-row>
        <mdb-row class="mt-4">
          <mdb-col col="3">
            <p class="p-0 m-0 mt-2" style="font-size: 14px">{{ $t("kassa_num") }}</p>
          </mdb-col>
          <mdb-col col="5">
            <mdb-input
              class="m-0 p-0"
  ...
  async created() {
    if (this.id > 0) {
      const res = await fetch(
        this.$store.state.hostname + "/TegirmonKassa/" + this.id
      );
      const data = await res.json();
      this.name = data.name;
      this.kassa_num = data.num_1;
