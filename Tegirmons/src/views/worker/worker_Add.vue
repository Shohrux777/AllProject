...
export default {
  ...
  async created()
  {
    if(this.id > 0)
    {
      const res = await fetch(this.$store.state.hostname + '/TegirmonWorker/' + this.id);
  ...
  methods: {
    ...
    save_data :  async function(){
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/TegirmonWorker", requestOptions);
