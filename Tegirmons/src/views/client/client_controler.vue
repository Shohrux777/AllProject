<template>
  <div>
    <div class="border-bottom d-flex justify-content-between main_header_bg_shadow">
      <router-link to="/client_controler">
         <h5 class="m-0 ml-4 d-flex" style="padding: 15px 0px; font-size: 20px;">
            {{$t('client_controler')}}</h5>
      </router-link>
      <div>
         <router-link to="/client_controler_Add/0">
    ...
    <anyTable v-else
      :datasource="all_client_controler"
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['client_controler_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/client_controler_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/TegirmonClientGroupControler/" + del_data.id, requestOptions);
          ...
          if(response.status == 201 || response.status == 200)
          {
            ...
            this.client_controler_delete_row(index);
          }
          else{
