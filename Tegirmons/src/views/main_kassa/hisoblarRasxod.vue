...
export default {
  ...
  computed: mapGetters(['allWorker', 'hisob_info']),
  async mounted(){
  ...
  methods: {
    ...mapActions(['fetchWorker', 'fetchHisob_info']),
    clw_rw(){
      this.rasxod = '0';
      this.rasxod_qty = 0;
      this.client_data = {};
    ...
    async saveRasxod(){
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
          ...
          "image_url": '0',
          "status_rasxod": 0,
          "tegirmonHisoblarid": this.hisob_id
      ...
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/TegirmonHisoblarRasxod", requestOptions);
