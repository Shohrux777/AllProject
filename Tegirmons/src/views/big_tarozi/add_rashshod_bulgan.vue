    ...
    <div  v-else class="add_tarozi_main px-3">
      <div class="add_tarozi_item rounded shadow mt-3 px-2 pt-3 pb-2" v-for="(item, index) in product_list" :key="index">
        <div class="row">
          ...
          <div class="col-2 mt-2">
            <mdb-input class="m-0 p-0 w-100 alert-warning" v-model="item.car_measure" disabled
          ...
          <div class="col-2 mt-2">
            <mdb-input class="m-0 p-0 w-100 alert-warning" v-model="item.discount" disabled
        ...
        <div class="photo mt-0 d-flex justify-content-start">
          ...
          <img :src="hostname + item.image_last_pull" width="120" alt="" v-show="item.image_last_pull" class="shadow border rounded ml-2">
        </div>
      </div>
    </div>
    <div class="add_container text-right border-top mt-3 mx-3 pt-1">
      <mdb-btn class="m-0 py-2 px-4" @click="save_items"
          style="font-size: 8px;" color="info"
          size="sm"><mdb-icon fas class="mr-1"  icon="print"></mdb-icon> {{$t('check')}}
        </mdb-btn>
    </div>
...
export default {
  ...
  methods: {
      ...
      try{
        ...
        if(res.status == 200 || res.status == 201){
          ...
          let item = {
            ...
            image_last_pull: data.image_last_pull,
    ...
    async save_items(){
      ...
      this.loading = false;
      this.$router.push('/rashshod_qilingan');
    },
    async save_item_index_changed(index, item_list){
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
          ...
          "image_last_pull": this.product_list[index].image_last_pull,
    ...
    takePhoto(data){
      this.product_list[this.photo_index].image_last_pull = data;
