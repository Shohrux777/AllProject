...
export default {
    ...
    methods: {
      ...
      async enter_invoice_new(invoice_id){
        this.$router.push('add_rashshod/' + invoice_id)
      },
      async updateList(){ 
        ...
        try{
        this.loading = true;
        const res = await fetch(this.$store.state.hostname + '/TegirmonInvoiceNews/getPaginationRashshodQilinmaganlarList?page=0&size=1000&b_date=' + b_date + '&e_date=' + e_date);
