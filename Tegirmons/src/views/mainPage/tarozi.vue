<template>
  <div class="taroziPage" :class="{'taroziPage_bg_color': clients_info.length}">
    <loader v-if="loading" />
    <div v-else class="main_operation" >
      <div class="header_shafyor ">
        <div class="mx-3">
          <div class="row px-2 pt-3  main_header_bg_new"> 
            ...
            <div class="col-3">
              <input class="m-0 py-0 bg-white rounded form-control" v-model="header.note"   outline 
                style="height:34px; font-size: 14px;" type="textarea" validate error="wrong" success="right"/>
              <small
                class="font-weight-bold px-2 py-0 header_input_title"
                >{{ $t("note") }}</small
              >
            ...
            <div class="col-3">
              <div class="text-right">
                <mdb-btn   @click="show_rasxod = true" color="danger" m="r2" style="font-size: 8.5px; margin-top: 3px;"
                  p="r3 l3 t2 b2"> <mdb-icon fas class="mr-1 "  icon="sync"></mdb-icon>  Расход
                </mdb-btn>
                <mdb-btn color="primary"  @click="addClientRow" style="font-size: 8.5px; margin-top: 3px;"
                p="r3 l3 t2 b2">
                <mdb-icon fas class="mr-2"  icon="user"/>{{$t('add_client')}}</mdb-btn>

                  <mdb-btn color="success"  @click="addRows" style="font-size: 8.5px; margin-top: 3px;"
                  p="r3 l3 t2 b2">
      ...
      <div class="row  pb-2 pt-3 mt-3 px-2 mx-2 tarozi_items rounded border bg-white" v-for="(item,indexRow) in clients_info" :key="indexRow">
        <div class="col-3">
          <erpSelectFio
        ...
        <div class="col-3">
          <input class="m-0 p-0 px-2 form-control" v-model="item.qty" @click="select_qty_Input(indexRow)" @blur="blur_qty_Input(indexRow)" size="md"  type="number" step="0.01" style="width:100%; height:35px;" />
        ...
        <div class="col-3 mt-3">
          <mdb-input class="m-0 p-0" v-model="item.note" size="md" :rows="1" outline group type="textarea" validate error="wrong" success="right"/>
          <small
            style="position: absolute; top: -7px; left: 20px; font-size: 11px"
            class="bg-white px-2 py-0"
            >{{ $t("reason") }}</small>
    ...
    <div class="text-right mr-4 mt-2" v-if="clients_info.length">
    ...
    <Toast ref="message" style="margin-top: 100px;"></Toast>
...
import erpSelectOnlyBugdoy from "../../components/erpSelectOnlyBugdoy";
...
export default {
  ...
  async mounted(){
    ...
    console.log(this.all_product_t)
    await this.selectMainProduct()
  },
  computed: mapGetters([ 'allClient', 'all_product_t']),
  methods: {
    ...mapActions(['fetchClient', 'fetch_product_t']),
    async selectMainProduct(){
      for(let i=0; i<this.all_product_t.rows.length; i++){
        if(this.all_product_t.rows[i].inv_accepted_status){
          this.only_product.name = this.all_product_t.rows[i].name;
          this.only_product.id = this.all_product_t.rows[i].id;
          this.only_product.price = this.all_product_t.rows[i].price;
        }
      }
    },
    ...
    select_qty_Input(i){
      if(this.clients_info[i].qty == 0){
        this.clients_info[i].qty = null;
      }
    },
    blur_qty_Input(i){
      if(this.clients_info[i].qty == null || this.clients_info[i].qty == ''){
        this.clients_info[i].qty = 0;
      }
    },
    ...
    async addClientRow(){
      ...
      let info = {
        ...
        client_id: null,
        client_new_name: '',
        product_id: this.only_product.id,
        product_name: this.only_product.name,
        product_price: this.only_product.price,
        ruyxat_id: 0,
        note: '',
        skidka: 0,
        qty: '0',
        ...
        if(this.clients_info.length<2){
          ...
          try{
            ...
            if(response.status == 201 || response.status == 200)
            {
              ...
              console.log(group_data)
              this.shafyorGroup_id = group_data.id;
              this.$refs.message.success('Added_successfully')
              return true;
            }
            else{
    ...
    async groupUpdateMeasure(measure){
      const requestOptions = {
        method : "POST",
        headers: { "Content-Type" : "application/json" },
        body: JSON.stringify({
    ...
    async clearAll(){
      ...
      var numTemp = 0
      for(let i=0; i<this.clients_info.length; i++){
        numTemp += parseFloat(this.clients_info[i].qty)
      }
      await this.groupUpdateMeasure(numTemp);
...
.tarozi_items{
  box-shadow: rgba(63, 63, 63, 0.24) 0px 3px 5px;
}
.header_input_title{
  position: absolute;
  top: -15px; 
