<template>
 <div class="mainShowGroup pt-0 px-1">
    ...
    <div v-else class="groupList">
      ...
      <div class="container-fluid mb-2" v-if="invoice_list.length">
        <div class="row ">
          <div class="col-6 m-0 px-2">
            <div class="d-flex  align-items-center">
              <small style="font-size: 12px;">Кассир: </small>
              <small style="font-size: 10px;" class="ml-4 mt-1 font-weight-bold">{{invoice_list[0].kassir}}</small>
            </div>
          </div>
          <div class="col-6 m-0 px-2">
            <div class="d-flex  align-items-center justify-content-end">
              <small style="font-size: 12px;">Дата: </small>
              <small style="font-size: 10px;" class="ml-4 mt-1 font-weight-bold">{{invoice_list[0].date.slice(8,10) + '-' + invoice_list[0].date.slice(5,7) + '-' + invoice_list[0].date.slice(0,4)}} <span class="ml-1">{{invoice_list[0].date.slice(11,16)}}</span> </small>
            </div>
          </div>
        </div>
      </div>
      ...
      <mdb-row class="mt-0 mx-2">
        <mdb-col col="12">
          <div class="mt-0 mr-3 text-right">
            <mdb-btn color="primary" v-show="invoice_list.length == 0"  @click="delGrroupEmpty" style="font-size: 10px"
              p="r4 l4 t2 b2">
            <mdb-icon  />{{$t('clear')}}</mdb-btn>
...
export default {
  ...
  methods: {
      ...
      for(let i=0; i<data.length;i++){
        ...
        try{
          ...
          if(resp.status == 201 || resp.status == 200){
            info.summ = data.summ;
            // if(data.tegirmonAuthid != null){
            //   info.kassir = data.auth.user.fio;
            // }
    ...
    async only_all_zaxira(){
      let qty_massa = 0;
      let zaxira_massa = 0;
      for(let i=0; i<this.invoice_list.length; i++){
        qty_massa += parseFloat(this.invoice_list[i].qty);
        zaxira_massa += parseFloat(this.invoice_list[i].zaxira_qty);
      }
      if(qty_massa <= zaxira_massa + 0.5 && qty_massa >= zaxira_massa - 0.5){
    ...
    async sendAll(){
      ...
      for(let i=0; i<this.invoice_list.length; i++){
        ...
        if(ostatka_measure >= 1){
          this.modal_info = (i+1) + ' - Ест остатка масса'; 
          this.modal_status = true;
          return;
        }
    ...
    async groupUpdateMeasure(){
      const requestOptions = {
        ...
        headers: { "Content-Type" : "application/json" },
        body: JSON.stringify({
          "shafyor_name": this.group_data.shafyor_name,
          "car_number": this.group_data.car_number,
          "note": this.group_data.note,
          "real_qty": this.group_data.real_qty,
          'qabul_qilgan_user_name': this.group_data.qabul_qilgan_user_name,
          'reverced_str': this.group_data.reverced_str,
