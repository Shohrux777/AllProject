<template>
  <div class="info_davernost p-4" v-if="option">
    <div class="row mt-2">
      <div class="col-3">
        <p class="p-0 m-0 mt-2 font-weight-bold" style="font-size: 14px;">{{$t('id')}}</p>
      </div>
      <div class="col-9 border-bottom">
        <p class="p-0 m-0 mt-2 text-primary font-weight-bold" style="font-size: 14px;">{{option.id}}</p>
      </div>
    </div>
    <div class="row mt-2">
      <div class="col-3">
        <p class="p-0 m-0 mt-2 font-weight-bold" style="font-size: 14px;">{{$t('check_number')}}</p>
      </div>
      <div class="col-9 border-bottom">
        <p class="p-0 m-0 mt-2 font-weight-bold" style="font-size: 14px;">{{option.check_number}}</p>
      </div>
    </div>
    <div class="row mt-2">
      <div class="col-3">
    ...
    <div class="row mt-2">
      ...
      <div class="col-9 border-bottom">
        <p class="p-0 m-0 mt-2" style="font-size: 14px;"><span >{{option.updated_date_time.slice(8,10) + '-' + option.updated_date_time.slice(5,7) + '-' + option.updated_date_time.slice(0,4)}}</span> <span class="ml-2">{{option.updated_date_time.slice(11,16)}}</span></p>
      </div>
    </div>
     <div class="row mt-2">
      <div class="col-3">
      ...
      <div class="col-9 border-bottom">
        <p class="p-0 m-0 mt-2 text-success" v-if="option.auth" style="font-size: 14px;">{{option.auth.user.fio}}</p>
        <p class="p-0 m-0 mt-2 text-success" v-else style="font-size: 14px;">Foydalanuvchi topilmadi</p>
    ...
    <div class="row mt-2">
      <div class="col-3">
        <p class="p-0 m-0 mt-2 text-danger" style="font-size: 14px;">Старый остаток</p>
      </div>
      <div class="col-9 border-bottom">
        <p class="p-0 m-0 mt-2 text-danger" style="font-size: 14px;">{{(parseFloat(option.credit_sum) + parseFloat(option.qty_real)).toFixed(1)}} кг</p>
      </div>
    </div>
    <div class="row mt-2">
      <div class="col-3">
        <p class="p-0 m-0 mt-2 text-success" style="font-size: 14px;"> Новый {{$t('ostatka')}}</p>
      </div>
      <div class="col-9 border-bottom">
        <p class="p-0 m-0 mt-2 text-success" style="font-size: 14px;">{{option.credit_sum.toFixed(1)}} кг</p>
      </div>
    </div>
    ...
    <div class="px-0 ">
      <table class="myTableInvoicelist mt-4">
        <thead>
          <tr class="header table_header_bg py-3">
            <th  width="40" class="text-left">№</th>
            <th>{{$t('product')}}</th>
            <th>{{$t('qty')}}</th>
            <th>{{option.product.name}}</th>
            <th>{{$t('summ')}}</th>
            <th>{{$t('date')}}</th>
        ...
        <tbody>
          <tr v-for="(row,rowIndex) in option.item_list" :key="rowIndex" :style="{'background': row.product.shitrix_code}">
          ...
          <tr v-show="option.summ" class="alert-success">
            <td> <span >$</span> </td>
            <td> <span >{{option.product.name}}</span> </td>
            <td> <span >{{option.dolg_summ}} кг</span> </td>
            <td> <span >{{option.dolg_summ}} кг</span> </td>
            <td><span>{{option.summ}} сум</span></td>
            <td> <span >{{ option.updated_date_time.slice(8,10) + '-' + option.updated_date_time.slice(5,7) + '-' + option.updated_date_time.slice(0,4)}}</span> <span class="ml-2">{{option.updated_date_time.slice(11,16)}}</span> </td>
          </tr>
        </tbody>
      </table>
    </div>
