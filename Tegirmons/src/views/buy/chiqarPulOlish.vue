  ...
  methods: {
    ...
    async saveRasxod(){
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
          "worker_name": this.client_info.name,
          "TegirmonContragentId": this.client_info.id,
    ...
    async getQarzUserId(){
      try{
        const response = await fetch(this.$store.state.hostname + "/TegirmonContragent/" + this.client_info.id,);
        if(response.status == 201 || response.status == 200)
        {
          const data = await response.json();
