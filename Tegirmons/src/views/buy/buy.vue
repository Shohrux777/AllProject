<template>
  <div>
    ...
    <anyTable v-else
      ...
      :status="status_table"
    />
    <modal-train  :show="item_status" headerbackColor="info"  titlecolor="white" title="Приход история" 
...
export default {
  ...
  methods: {
    ...
    async fetchUserAccess(id){
      ...
      async refresh(){
        const res = await fetch(this.$store.state.hostname + '/TegirmonInvoice/getPaginationBugdoyPrixodQilinganTovarlarListiVaqtsz?page=' + 0 + '&size=' + 10000);
