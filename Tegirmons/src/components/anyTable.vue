    ...
    <loader v-if="loading"/>
      ...
      <table  class="myTable mt-1">
        ...
        <tr class="header ">
            ...
            <th width="140" class="text-center">{{$t('Action')}}</th>
        </tr>
        </thead>
        <tbody>
          <!-- .slice().reverse() -->
        <tr v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" >
            ...
            <td class="text-center" v-if="buy_prixod">
              <i class="fas fa-pen editIcon mask waves-effect  m-0 pr-2" :class="{'applied': disable, 'applied': row['inv_accepted_status']}" v-on:click="editRow" :data-row="rowIndex"></i>
              <i class="fas fa-trash delIcon mask waves-effect m-0 pl-2" :class="{'applied': disable, 'applied': row['inv_accepted_status']}" v-on:click="deleteRow" :data-row="rowIndex"></i>
            </td>
            <td class="text-center" v-else>
              <i v-if="userLock" style="color: #37474F;" class="fas fa-wrench mask waves-effect m-0 pr-3" :class="{'applied': disable}" v-on:click="AccessRow" :data-row="rowIndex"></i>
              ...
              <i class="fas fa-pen editIcon mask waves-effect  m-0 pr-2" :class="{'applied': disable && !status}" v-on:click="editRow" :data-row="rowIndex"></i>
  ...
  methods: {
    ...
    async checklist (option){
      ...
      paginationClick(i){
        ...
        AccessRow(ev){
          this.num_target = ev.target.dataset.row;
          this.$emit('for_access',this.datasource.rows[this.num_target]);
        },
        deleteRow(ev) {
            this.confirm = true
        ...
        LockRow(ev){
          ...
          this.$router.push('/authorization/' + this.datasource.rows[this.num_target].id)
