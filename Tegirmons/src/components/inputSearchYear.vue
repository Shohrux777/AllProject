...
export default {
  ...
  methods: {
    ...
    async search(){
      ...
      var x = this.name.replace(/\D/g, '').match(/(\d{0,2})(\d{0,2})(\d{0,4})/);
      // this.name = !x[2] ? x[1] : + x[1] + '-' + x[2] + (x[3] ? '-' + x[3] : '');
      if(this.name.length == 2){
        this.name = x[1];
      }
      else if(x[1].length >= 2){
        this.name = x[1] + '-' + x[2] + (x[3] ? '-' + x[3] : '');
      }
      // --> regex inputni tugilgan yil formatiga utkazish dd-mm-yyyy
      ...
      var search_birthday = '';
      if(this.name.length <= 2){
        search_birthday = this.name
      }
      else if(this.name.length >= 5 && this.name.length < 10){
        search_birthday = this.name.slice(3,5) + '-' + this.name.slice(0,2)
      }
      else{
        search_birthday = this.name.slice(6,11) + '-' + this.name.slice(3,5) + '-' + this.name.slice(0,2);
      }
      // --> regexdan qidirish formatiga utkazish yyyy-mm-dd

      try{
        if(this.url != '' && this.name != ''){
          const res = await fetch(this.$store.state.hostname + this.url + search_birthday);
