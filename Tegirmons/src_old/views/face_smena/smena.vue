...
export default {
  ...
  computed: mapGetters(['get_smena_list']),
  methods: {
    ...mapActions(['fetch_Smena', ]),
    ...mapMutations(['smena_row_delete']),
  
      for_edit(edit_data)
      {
        this.$router.push("/smena_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/SkudSmenas/" + del_data.id, requestOptions);
