...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['salary_row_delete']),
  
      for_edit(edit_data)
      {
        this.$router.push("/salary_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/SkudOyliks/" + del_data.id, requestOptions);
