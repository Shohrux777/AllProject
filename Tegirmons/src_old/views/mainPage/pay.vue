...
export default {
  ...
  created() {
    this.$root.$refs.payedZaxira = this;
  },
  mounted() {
  ...
  methods: {
    ...
    this.$nextTick(function () {
      ...
      this.persantage_discount_default = parseFloat(0);
      ...
      console.log('this.check_id', this.check_id)
      if(this.check_id>0){
        try{
    ...
    async payed(){
      console.log('this.changeProduct', this.changeProduct)
      ...
      try{
        ...
        for(let i=0; i<this.changeProduct.length; i++){
          if(this.changeProduct[i].sum_status == true && this.changeProduct[i].enoughQty>0){
            let zaxira_temp = {
              tegirmonProductid: this.changeProduct[i].product_id,
              qty: parseFloat(this.changeProduct[i].enoughQty).toFixed(1),
              summa: (parseFloat(this.changeProduct[i].enoughQty) * parseFloat(this.changeProduct[i].product_price)).toFixed(),
              profit_summ: this.changeProduct[i].product_price,
              auth_user_creator_id: localStorage.AuthId,
            }
            console.log('zaxira_temp', zaxira_temp)
            orderList.push(zaxira_temp);
          }
        }
      ...
      const requestOptions = {
        ...
        body: JSON.stringify({
          ...
          "id": this.check_id,
          "tegirmonClientid": 1, // bu zaxiradan check yozilganini bildiradi
          "tegirmonContragentid": 1,
