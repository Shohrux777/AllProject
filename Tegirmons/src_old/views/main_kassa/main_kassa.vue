<template>
  <div class="bg-white">
          ...
          <table class="mainKassaTable">
            ...
            <tbody>
              <tr v-for="(row,rowIndex) in get_payment_list" :key="rowIndex" 
                ...
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;">{{row.worker_name}}</small> </td>
                ...
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;" :class="{'text-success': row.status_rasxod == 1, 'text-danger': row.status_rasxod == 0}">{{row.all_summ_string}}</small> </td>
                <td @click="showRasxodItem(row)" v-if="row.status_rasxod == 1"> <small style="font-size: 11.5px;" class="text-success">{{row.image_url}}</small> </td>
                <td @click="showRasxodItem(row)" v-else> <small style="font-size: 11.5px;" class="text-danger">{{row.reserve}}</small> </td>
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;" :class="{'text-success': row.status_rasxod == 1, 'text-danger': row.status_rasxod == 0}">{{row.dollor_string}}</small> </td>
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;" v-if="row.kurs">{{row.kurs.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</small> </td>
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;">{{row.addiotionala_information}}</small> </td>
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;" v-if="row.created_date">{{row.created_date.slice(8,10) + '-' + row.created_date.slice(5,7) + '-' + row.created_date.slice(0,4)}}</small> <small class="ml-2">{{row.created_date.slice(11,16)}}</small> </td>
                <td @click="showRasxodItem(row)"> <small style="font-size: 11.5px;">{{row.note}}</small> </td>
                <td class="text-center" @click.stop="openEditModal(row)">
                  <mdb-icon 
                    v-if="isToday(row.created_date)"
                    icon="edit"
                    class="text-warning"
                    style="cursor: pointer; font-size: 14px;"
                    title="Izminit qilish"
                  />
                </td>
              </tr>
            </tbody>
    ...
    <modal-train  :show="rasxod_show" headerbackColor="#fc4640"  titlecolor="black" :title="editMode ? 'Izminit qilish - Расходъ' : $t('rasxod')" 
      @close="closeRasxod" width="65%">
      <template v-slot:body>
        <rasxod 
          @close="closeRasxod" 
          :cash_kassa="today_cash + yest_day_cash" 
          :dollor_kassa="today_dollor + yest_day_dollor" 
          :editData="editRasxodData"
          :isEditMode="editMode"
          ref="rasxodWorkerSum">
        </rasxod>
      </template>
  ...
  <modal-train  :show="pul_olib_qolish" headerbackColor="#009587"  titlecolor="black" :title="editMode ? 'Izminit qilish - Получать деньги' : $t('pul_olish')" 
      @close="closePulChiqish" width="65%">
      <template v-slot:body>
        <chiqarPulOlish 
          @close="closePulChiqish" 
          :editData="editRasxodData"
          :isEditMode="editMode"
          ref="prixodWorkerSum">
        </chiqarPulOlish>
    ...
    methods: {
      ...
      isToday(dateString) {
        ...
        try {
          ...
          console.error('isToday error:', e, dateString);
          return false;
        }
      },
      openEditModal(row) {
        console.log('openEditModal called', row);
        console.log('created_date:', row.created_date);
        console.log('isToday result:', this.isToday(row.created_date));
        if (!this.isToday(row.created_date)) {
          this.$refs.message.warning('Faqat bugungi kundagi rasxod/prixod ni izminit qilish mumkin!');
          return;
        }
        this.editMode = true;
        this.editRasxodData = row;
        if (row.status_rasxod == 0) {
          // Rasxod modalini ochish
          console.log('Opening rasxod modal');
          this.rasxod_show = true;
        } else {
          // Prixod modalini ochish
          console.log('Opening prixod modal');
