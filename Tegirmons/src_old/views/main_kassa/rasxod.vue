          ...
          <div  v-for="(item,i) in client_list.rows" :key="i" class="item px-3" 
            ...
            <div>
              <div class="d-flex">
                ...
                <div class="px-3">
                  <p class="m-0 p-0 font-weight-bold" style="font-size: 11px;">{{item.fio}}</p>
                  <p class="m-0 mt-1 rangRasxod" style="font-size:10px;">Год: <span v-if="item.addiotionala_information" class="px-2">
      ...
      <div style="width: 50%;">
        <div  class="row pl-3">
          <div class="col-12 mt-0">
            <input class="m-0 px-2 form-control" v-model="client_name" @keypress="change_klient_name()"
...
export default {
  ...
  methods: {
    ...
    loadEditData(data) {
      this.client_name = data.worker_name || '';
      this.client_id = data.TegirmonWorkerid || null;
      this.rasxod_qty = data.rasxod || 0;
      // Formatlash uchun funcRasxod ni chaqiramiz
      if (this.rasxod_qty > 0) {
        this.rasxod = this.rasxod_qty.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ');
      } else {
        this.rasxod = '0';
      }
      this.dollor = data.dollor || 0;
      // Formatlash uchun funcDollor ni chaqiramiz
      if (this.dollor > 0) {
        this.dollor_string = this.dollor.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ');
      } else {
        this.dollor_string = '0';
      }
      this.dollor_kurs_qty = data.kurs || 0;
      if (this.dollor_kurs_qty > 0) {
        this.dollor_kurs = this.dollor_kurs_qty.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ');
      } else {
        this.dollor_kurs = '0';
      }
      this.note = data.note || '';
      this.photo_url = data.phone_number || '';
      this.all_summ = data.all_summ || 0;
      
      // Worker ni topish
      if (this.client_id && this.allWorker.rows) {
        const worker = this.allWorker.rows.find(w => w.id === this.client_id);
        if (worker) {
          this.client_data = worker;
          this.search = worker.fio;
          const index = this.allWorker.rows.findIndex(w => w.id === this.client_id);
    ...
    async saveRasxod(){
      ...
      const requestBody = {
        "worker_name": this.client_name,
        "tegirmonWorkerid": this.client_id,
        "passport_number": this.client_data.addiotionala_information,
        ...
        "rasxod" : this.rasxod_qty,
        "prixod": 0,
        "reserve": this.rasxod,
        "image_url": '0',
        "status_rasxod": 0,
        "main_kassa_status": true,
        "auth_user_updator_id": this.editData?.auth_user_updator_id || 0
      };
      ...
      const url = this.isEditMode 
        ? `${this.$store.state.hostname}/TegirmonMainKassaRasxod/izminit/${this.editData.id}`
        : `${this.$store.state.hostname}/TegirmonMainKassaRasxod`;
      ...
      try{
        ...
        if(response.status == 201 || response.status == 200)
        {
          ...
          this.$emit('close');
          this.loading = false;
          this.$refs.message.success('Muvaffaqiyatli saqlandi!');
          return true;
        }
        else{
          this.modal_info = data?.message || this.$i18n.t('network_ne_connect');
