    ...
    <div class="getProduct">
      <div class="container-fluid">
        <div class="row px-3 py-2 d-flex align-items-center main_header_bg_new" :class="{'bg_red_color': oshibka_client == 1}">
          ...
          <div class="col-3">
            <input-search KeyName="name"  @select="selectClientPassport" :label="$t('passport_number')" :selected="passport_number"
              url="/TegirmonContragent/getPaginationSearchByFioOrPassportSerailNumberOrHomeOrMobilePhoneNumber?page=0&size=100&fio_or_serial_number="
          ...
          <div class="col-3">
            <input-search-year KeyName="name"  @select="selectClientBorn" type="text" :label="$t('born_date')" :selected="born_date" placeholder="дд-мм-гггг"
              url="/TegirmonContragent/getPaginationByBornDate?page=0&size=100&born_date_str="
      ...
      <div class="mainWorkChange mt-1">
        <div class="d-flex justify-content-between px-2">
            <div class="w-100 p-2">
                <div class="row equal-height">
                  <div class="col-5" >
                    <div class="card" v-if="client_info.name" :class="{'alert-danger': client_info.sum>0 || client_info.dollor>0, 'alert-success': client_info.sum<0 || client_info.dollor<0}">
                      <div class="d-flex w-100">
                        ...
                        <div class="user_info_selected pt-4 px-3">
                          <h5 class="font-weight-bold" style="font-size: 14.5px;">{{client_info.name}}</h5>
                      ...
                      <div class="ishlagan_puli d-flex align-items-center justify-content-between py-2 px-5 " style="border-top: 1px solid grey">
                          <h5 class="font-weight-bold m-0 text-danger" style="font-size: 17px;" v-if="client_info.sum>0 || client_info.dollor>0">Qarz :</h5>
                          <h5 class="font-weight-bold m-0 text-success" style="font-size: 17px;" v-else>Oldindan to'lov :</h5>
                          <h5 v-if="client_info.sum>0" class="font-weight-bold ml-3 m-0" style="font-size: 25px; color: brown;">{{ client_info.sum.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')  }} сўм</h5>
                          <h5 v-else class="font-weight-bold ml-3 m-0" style="font-size: 25px; color: green;">{{ (-1*client_info.sum).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')  }} сўм</h5>
                          <h5 v-if="client_info.dollor>0" class="font-weight-bold ml-3 m-0" style="font-size: 25px; color: brown;">{{client_info.dollor.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}} $</h5>
                          <h5 v-else class="font-weight-bold ml-3 m-0" style="font-size: 25px; color: green;">{{(-1*client_info.dollor).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}} $</h5>
                      </div>
        ...
        <div class="px-0 userSalaryTable">
          ...
          <table v-else class="myTableuserSalaryList mt-2">
            ...
            <tbody>
              ...
              :class="{'zero_item': row.status_rasxod == 0}">
                <td> <small >{{rowIndex+1}}</small> </td>
                <td> <small v-if="row.invoice_id">{{row.invoice_id}}</small></td>
                <td> <small >{{user_name}}</small> </td>
                <td> <small :class="{'text-danger': row.status_rasxod == 0, 'text-success': row.status_rasxod == 1}">{{row.sum_str}}</small></td>
                <td> <small :class="{'text-danger': row.status_rasxod == 0, 'text-success': row.status_rasxod == 1}">{{row.dollor_string}}</small></td>
                <td> <small >{{row.kurs.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</small></td>

                <td v-if="row.status_rasxod == 1"> <span class="status_btn_bg px-2 text-white rounded" style="padding: 1px 6px; font-size: 10px;">Pul berish </span> </td>  
                <td  v-else-if="row.status_rasxod == 0"> <span class="bg-danger px-2 text-white rounded" style="padding: 2px 10px; font-size: 10px;">Tovar olish</span></td>
                ...
                <td> 
                  <small v-if="row.reserve_val_1>0" :class="{'text-danger': row.reserve_val_1>0}" >{{row.reserve_val_1.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</small>
                  <small v-else class="text-success"  >{{(-1 * row.reserve_val_1).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</small>
                </td>
                <td> 
                  <small v-if="row.reserve_val_2>0" :class="{'text-danger': row.reserve_val_2>0}" >{{row.reserve_val_2.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</small>
                  <small v-else class="text-success" >{{(-1 * row.reserve_val_2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')}}</small>
                </td>
...
//     },
  ...
  methods: {
    ...
    // skud userdan qarzdolik userga malumotlarni utkazish
    ...
    async fetchUserPrixodRasxod(){
      try{
        const response = await fetch(this.$store.state.hostname + "/TegirmonContragentRasxod/getPaginationUserId?page=0&size=200&userid=" + this.client_info.id);
        const data = await response.json();
        if(response.status == 201 || response.status == 200)
        {
...
.zero_item{
  background: linear-gradient(to right, #ffc9d6, #ffdad2);
}
.rounded{
  border-radius: 48%;
}
