...
export default {
  ...
  computed: mapGetters(['all_contragent_t']),
  methods: {
    ...mapActions(['fetch_contragent_t', ]),
    ...mapMutations(['contragent_t_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/contragent_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
          ...
          if(response.status == 201 || response.status == 200)
          {
            ...
            this.contragent_t_delete_row(index);
          }
          else{
