<template>
  <div class="getProduct">
                            ...
                            <tbody>
                                <tr v-for="(row,rowIndex) in searchlist" :key="rowIndex" v-show="row.currentQty && (row.currentQty - row.kirim + row.chiqim) >0"
                                  ...
                                  :class="{'ostatka_not_background': row.currentQty<=0}">
                                    ...
                                    <td> 
                                      <span 
                                        @click="openProductHistoryModal(row, 1)"
                                        style="cursor: pointer; text-decoration: underline; color: #4caf50; font-weight: 600;"
                                    ...
                                    <td> 
                                      <span 
                                        @click="openProductHistoryModal(row, 2)"
                                        style="cursor: pointer; text-decoration: underline; color: #f44336; font-weight: 600;"
                                        title="Batafsil ko'rish"
                                      >
                                        {{
                                    ...
                                    <td v-show="sklad_diff" > 
                                      <input type="text" :value="row.new_qty_str" @keyup="funcDiffQty($event.target.value, rowIndex)" 
                                        @keydown="handleTabKey($event, rowIndex)"
      ...
      <modal-train 
        :show="productHistoryModalShow" 
        headerbackColor="white" 
        titlecolor="black" 
        :title="productHistoryModalTitle" 
        @close="productHistoryModalShow = false" 
        width="80%"
      >
        <template v-slot:body>
          <div v-if="productHistoryLoading" class="text-center p-4">
            <div class="spinner-border" role="status">
              <span class="sr-only">Yuklanmoqda...</span>
            </div>
          </div>
          <div v-else class="product-history-table-wrapper">
            <table class="product-history-table">
              <thead>
                <tr class="product-history-table-header">
                  <th width="50">№</th>
                  <th>Sklad</th>
                  <th>Type</th>
                  <th>Miqdor</th>
              ...
              <tbody>
                <tr v-for="(history, index) in productHistoryList" :key="history.id" class="product-history-table-row">
                  <td>{{ index + 1 }}</td>
                  <td>{{ history.skladName }}</td>
                  <td>
                    <span :class="{
                      'badge badge-success': history.type === 1,
                      'badge badge-danger': history.type === 2,
                      'badge badge-info': history.type === 3 || history.type === 4
                    }">
                      {{ history.typeName }}
                    </span>
                  </td>
                  <td>
                    {{ formatNumber(history.qty).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ') }} {{ history.unitM }}
                  </td>
                  <td>
                    {{ formatNumber(history.price).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ') }} сум
                  </td>
                  <td>{{ history.userName || 'Noma\'lum' }}</td>
                  <td>{{ formatDate(history.createDate) }}</td>
                  <td>{{ history.note || '-' }}</td>
                </tr>
                <tr v-if="productHistoryList.length === 0" class="product-history-table-row">
  ...
  methods: {
    ...
    funcAllBlue(index){
      if(this.searchlist[index].new_qty_str == '' || this.searchlist[index].new_qty_str == null){
        this.searchlist[index].new_qty = this.searchlist[index].currentQty;
        this.searchlist[index].new_qty_str = this.searchlist[index].currentQty.toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ');
    ...
    handleTabKey(event, index){
      // TAB key kodi 9 ga teng
      if(event.keyCode === 9 || event.key === 'Tab'){
        // Agar input bo'sh bo'lsa yoki faqat probellar bo'lsa, currentQty ga tenglashtiramiz
        let inputValue = this.searchlist[index].new_qty_str;
        ...
        // TAB bosilganda funcAllBlue ni chaqiramiz
        // Bu inputdan chiqishdan oldin farqni hisoblaydi
        this.funcAllBlue(index);
        
        // TAB ning default xatti-harakatini to'xtatmaymiz, shunda keyingi input ga o'tadi
        // preventDefault chaqirmaymiz, shunda browser default TAB xatti-harakatini bajaradi
        // blur event ham avtomatik bajariladi
      }
    },
    funcDiffQty(n,index){
      console.log(n)
    ...
    async openProductHistoryModal(row, type) {
      // type: 1 - kirim, 2 - chiqim, null - hammasi
      this.productHistoryLoading = true;
      this.productHistoryModalShow = true;
      
      let typeName = type === 1 ? 'Kirim' : type === 2 ? 'Chiqim' : 'Barcha';
      this.productHistoryModalTitle = `${row.productName} - ${typeName} tarixi`;
      
      let url = this.$store.state.hostname + `/TegirmonSkladHistory/getProductHistoryDetails?product_id=${row.productId}`;
      
      // Sklad filter
      if (this.sklad_id) {
        url += `&sklad_id=${this.sklad_id}`;
      }
      
      // Date filter
      if (this.Start_time) {
        url += `&date=${this.Start_time}`;
      }
...
.product-history-table-header {
  background-color: #C4DEE4;
  position: sticky;
  top: 0;
  z-index: 10;
}
.product-history-table-header th {
  font-weight: 600;
  font-size: 12px;
  padding: 10px 12px;
  text-align: left;
...
.product-history-table-row {
  border-bottom: 1px solid #e0e0e0;
  transition: background-color 0.2s;
}
.product-history-table-row:hover {
  background-color: #f5f5f5;
}
.product-history-table-row td {
  padding: 8px 12px;
  font-size: 12px;
  color: #333;
}
