...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['door_delete_row']),
  
      for_edit(edit_data)
      {
        this.$router.push("/doors_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/SkudDoors/" + del_data.id, requestOptions);
