            ...
            <table class="change-table">
              ...
              <tbody>
                <tr v-for="(row,rowIndex) in get_payment_list" :key="rowIndex" class="change-table-row">
                  ...
                  <td><span class="change-table-cell">{{row.TegirmonProductid}}</span></td>
                  <td><span class="change-table-cell">{{row.qty_real}}</span></td>
                  <td>
  ...
  export default {
    ...
    methods: {
      ...
      async apply(){
        ...
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + "/TegirmonInvoice/getPaginationBugdoyTortilganTegirmondanUnUchunListi?page=0&size=500&b_date="  + b_date + '&e_date='+ e_date);
          this.loading = false;
