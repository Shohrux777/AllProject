  ...
  <div class="d_table  p-2">
    <loader v-if="loading"/>
          ...
          <mdb-col class="col-12 col-sm-12 col-md-8 col-lg-7 mt-1">
              ...
              <div class="d-inline ">
                <mdb-btn-group  style="margin-top:-18px">
                    <mdb-dropdown>
                      <mdb-btn style="font-size: 13px;" class="mr-1 mt-1 text-primary" outline="none" darkWaves tag="a" floating
                  ...
                  <mdb-btn style="font-size: 13px;" class="mr-0 text-primary" outline="none" darkWaves tag="a" floating @click="showcheck_form=true"
                  icon="table"  size="sm">{{$t('editCol_columns')}}</mdb-btn>

                  <mdb-btn style="font-size: 13px;" slot="reference" class="mr-1 text-primary" hidden outline="none" @click="clicked_filter"
      ...
      <table  class="myTable mt-3">
                ...
                <tbody>
                  <!-- .slice().reverse() -->
                <tr v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" @dblclick="selectItem(row)">
                    <td>{{(rowIndex+1)}}</td>
                    ...
                    <td v-if="buy_prixod">
                      <mdb-btn class="mr-1 py-1 px-2" style="font-size: 7px;" :disabled="disable && !status" color="success" v-if="!row['inv_accepted_status']"  @click="applybuy" :data-row="rowIndex"
                        size="sm">{{$t('apply')}}
                      </mdb-btn>
                    </td>
                    <td class="text-center" v-if="buy_prixod">
                      <i class="fas fa-pen editIcon mask waves-effect  m-0 pr-2" :class="{'applied': disable && !status, 'applied': row['inv_accepted_status']}" v-on:click="editRow" :data-row="rowIndex"></i>
                      <i class="fas fa-trash delIcon mask waves-effect m-0 pl-2" :class="{'applied': disable && !status, 'applied': row['inv_accepted_status']}" v-on:click="deleteRow" :data-row="rowIndex"></i>
                    </td>
                    <td class="text-center" v-else>
                      <i v-if="userLock" class="fas fa-lock text-warning mask waves-effect m-0 pr-3"  v-on:click="LockRow" :data-row="rowIndex"></i>
                      <i  class="fas fa-pen editIcon mask waves-effect  m-0 pr-2" :class="{'applied': disable && !status, 'applied': row['applyment_status'] == true,}" v-on:click="editRow" :data-row="rowIndex"></i>
                      <i class="fas fa-trash delIcon mask waves-effect m-0 pl-2" :class="{'applied': disable && !status, 'applied': row['applyment_status'] == true,}" v-on:click="deleteRow" :data-row="rowIndex"></i>
                    </td>
  ...
  methods: {
    ...
    async checklist (option){
      ...
      paginationClick(i){
        ...
        applybuy(ev){
          this.num_target = ev.target.dataset.row
          this.$emit('accept', this.datasource.rows[this.num_target].id)
        },
