<template>
  <div class="erp-select">
    <div class="input_div d-flex align-items-center mb-0 p-0" @click="select_input" :class="{'vaeble': change,  'varible': change_color}" >
        <input  group  type="text" class="px-2"  @keyup="hideInput1" />
        <label for="text" class="p-0" style="margin-top: -6px; font-size: 12px;" v-if="selected == ''">
         {{$t('search_product')}} 
        </label>
        <label for="text" class="p-0" style="margin-top: -6px; color: #495057; font-size: 12px;" v-else>
          {{selected}}
         </label>
        <div class="icon_caret  p-0" style="margin-top: -2px;">
          <i class="fa fa-caret-down mx-2 mt-2 text-success"  ></i> 
        </div>
...
export default {
  ...
  methods: {
    ...
    selectOption(option)
    {
      this.$emit('select', {index: this.row, data:option});
