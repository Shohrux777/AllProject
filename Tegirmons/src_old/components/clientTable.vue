    ...
    <loader v-if="loading"/>
      ...
      <table  class="myTable mt-1">
        ...
        <tr class="header ">
            ...
            <th style="font-size: 11px;" width="100" class="text-center">{{$t('Action')}}</th>
        </tr>
        </thead>
        <tbody>
          <!-- .slice().reverse() -->
        <tr v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" @dblclick="selectItem(row)">
            <td>{{(rowIndex+1)}}</td>
            <td v-for="(column,i) in datasource.columns" :key="i" style="font-size: 11.5px;">
  ...
  methods: {
    ...
    async checklist (option){
      ...
      paginationClick(i){
        ...
        LockRow(ev){
          this.$router.push('/authorization/0')
          this.num_target = ev.target.dataset.row
