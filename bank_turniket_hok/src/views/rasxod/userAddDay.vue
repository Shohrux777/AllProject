...
export default {
    ...
    methods:{
      ...
      async deluser_day_data(del_id, user_name){
        if (confirm("Ты уверен? Удаляет рабочий день " + user_name) == true) {
            console.log(del_id)
            try{
                ...
                const response = await fetch(this.$store.state.hostname + "/SkudUserDayAdd/" + del_id, requestOptions);
      ...
      async submit(){
        ...
        try{
          this.loading = true;
          const response = await fetch(this.$store.state.hostname + "/SkudUserDayAdd/getPaginationGetByDateTimeUser_add_day?page=0&size=10000&begin_date=" + start + '&end_date=' + end);
          const data = await response.json();
          this.loading = false;
          this.reportList = []
          console.log('data') 
