    ...
    <div class="card options" v-if="areOptionsVisible">
      <div class="down_list">
        <div class="container my-2">
          <input :placeholder="$t('search_here')" m="b0 t3"
          v-model="search" :ref="filteredList[0].acc_name" class="input_search_erpselect"
          />
        </div>
  ...
  methods: {
    ...
    removeTag(){
      this.$nextTick(function () {
        this.$refs[this.filteredList[0].acc_name].focus();
        this.search = '';
      })
      ...
      filteredList: function(){
          if(this.search)
          {
            return this.options.filter((item)=>{
              return this.search.toLowerCase().split(' ').every(v => item.acc_name.toLowerCase().includes(v))
            })
          }else
          {
            return this.options;
