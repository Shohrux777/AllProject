export default {
    ...
    actions: {
        ...
        async fetchPostavchik(ctx) {
            const res = await fetch(ctx.rootState.hostname + '/PosCompanies/getPostavshikList');
            const res_data = await res.json();
            // console.log(ctx.rootState.hostname);
            ctx.commit('updatePostavchik', res_data);
        },
    ...
    mutations: {
        ...
        updatePostavchik(state, data) {
            console.log(data)
            state.m_postavchik.rows = data;
        },
