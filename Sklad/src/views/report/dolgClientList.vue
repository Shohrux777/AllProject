<template>
  <div>
    <div class="border-bottom d-flex justify-content-between align-items-center px-3 ">
      <h6 class="m-0 p-0">{{ $t('client_dolg') }}</h6>
    ...
    <div class="mx-3 py-1 p-0 border-bottom">
      <mdb-row>
        ...
        <mdb-col class="col-12 col-sm-12 col-md-8 col-lg-7 text-right">
          <mdb-btn type="submit" color="primary py-2 px-4 " @click="add_dolg" style="font-size:10px; margin-top: 9px;"  >
            {{$t('add_dolg')}}
          </mdb-btn>
    ...
    <div class="mainChart px-3" id="print">
      <table  class="myTable">
        ...
        <tbody>
          <tr v-for="(item,i) in filteredList" :key="i" @click="showCheckID(item.PosCheckid)">
            <td>{{i+1}}</td>
    ...
    <mdb-modal size="xl" :show="dolg_show" @close="dolg_show = false">
      <mdb-modal-header>
        <mdb-modal-title>{{$t('add_dolg')}}</mdb-modal-title>
      </mdb-modal-header>
      <mdb-modal-body>
        <add_dolg @close="dolg_close"/>
      </mdb-modal-body>
      <mdb-modal-footer>
        <mdb-btn color="secondary" size="sm" @click.native="dolg_show = false">Close</mdb-btn>
        <!-- <mdb-btn color="primary" size="sm">Save changes</mdb-btn> -->
      </mdb-modal-footer>
    </mdb-modal>
...
import add_dolg from '../aptekaSale/add_dolg.vue'
