<template>
  <div>
    <check-id-print v-if="show_print_check" :options="checkdata" :client_name="client_name" @close="show_print_check = false"/>
    <div v-else>
      <div class="border-bottom d-flex justify-content-between align-items-center px-3">
        <h6 class="m-0 p-0">{{ $t('check_list') }}</h6>
        <router-link to="/saled">
          <h6 class="m-0 ml-4 d-flex" style="padding: 11px 0px">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-chevron-left mr-1" width="18" height="18" viewBox="0 0 24 24" stroke-width="2.5" stroke="#00abfb" fill="none" stroke-linecap="round" stroke-linejoin="round">
        ...
        <table  class="myTable">
          <thead>
                <tr  class="header ">
                  ...
                  <th width="250">{{$t('client')}}</th>
                  <th >{{$t('check_id')}}</th>
                  <th>{{$t('total')}}</th>
                  <th>{{$t('Total')}}</th>
                  <th>{{$t('pay_dolg_id')}}</th>
                  <!-- <th>{{$t('perechisliniya')}}</th> -->
                  <th>{{$t('creditd_summ')}}</th>
                  <!-- <th>{{$t('summ')}}</th> -->
                </tr>
              </thead>
          <tbody>
            <tr v-for="(item,i) in allreports" :key="i" @click="showCheckID(item.id,item.client_name_str)">
              <td>{{i+1}}</td>
              <td>{{item.client_name_str}}</td>
              <td >{{item.id}}</td>
              <td>{{item.mobil_sum}}</td>
              <td class="text-success"> + {{item.summ}}</td>
              <td class="text-secondary">{{item.payme_sum}}</td>
              <!-- <td>{{item.discount_summ}} </td> -->
      ...
      <mdb-modal size="lg" :show="show_check" @close="show_check = false">
        <mdb-modal-header>
          <mdb-modal-title>Накладной</mdb-modal-title>
        </mdb-modal-header>
        <mdb-modal-body>
          <show_checked :options="checkdata" :client_name="client_name"/>
        </mdb-modal-body>
        <mdb-modal-footer>
          <mdb-btn color="info" style="font-size: 9px;" size="sm" @click.native="show_check = false">
            <mdb-icon class="mr-1" icon="file-excel" /> {{('excel')}}
          </mdb-btn>
          <mdb-btn color="primary" style="font-size: 9px;" size="sm" @click.native="printCheck">
            <mdb-icon class="mr-1" icon="print" /> {{$t('print')}}
          </mdb-btn>
...
import CheckIdPrint from '../sale/checkIdPrint.vue';
export default {
  data(){
    return {
      applyStatus: true,
  ...
  methods: {
    ...
    async showCheckID(id, client_name){
      if(client_name){
    ...
    pdf(){
      ...
      async refresh(){
        this.dateString = this.date + 'T00:00:00.000Z';
        this.date1String = this.date1 + 'T23:59:00.000Z';
        try{
          this.loading = true;
          const res = await fetch(this.$store.state.hostname + '/PosChecks/getCheckListByDateTime?page=' + this.activePage + '&size=' + this.value + '&begin_date=' + this.dateString + '&end_date=' + this.date1String);
