        ...
        <div class="d-flex mt-2">
          <div class="skidka " style="width:35%">
            <div style="position:relative;" class="text-left text-warning">
              <h6 class="m-0 p-0 mt-2">{{$t('perechisliniya')}}</h6>
            </div>
    ...
    async getAllSumm(){
      ...
      try{
        ...
        if(res_data.total_sum == null){
          ...
          this.totalIn = (res_data.total_sum).toFixed(2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.cashInString = res_data.cash_sum.toFixed(2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.uzcardInString = res_data.card_sum.toFixed(2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.humoInString = res_data.humo_sum.toFixed(2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.clickInString = res_data.discount_sum.toFixed(2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
          this.onlineInString = res_data.online_sum.toFixed(2).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
        }
