<template>
  <div class="addProductQtyPayed" >
    <div class="p-4 d-flex">
      <!-- v-show="false" -->
      <div class="client_list_pay" v-show="false">
      ...
      <div class="acceptBoxPayed px-3 py-2 text-center">
        <div class="py-1 border-bottom">
          <div class="text-center">
            <span style="font-size: 19px;">{{ client_name }}</span>
          </div>
        </div>
        ...
        <div class="d-flex mt-2">
          ...
          <div class="price ml-4 "  style="width:65%; position:relative;">
            ...
            <div style="position:relative;">
              ...
              class="form-control  border mt-4 text-right pr-2" style="border:none; outline:none;font-weight:bold; height:30px;">
              <small style="position:absolute; top:-16px; left:3px; font-size:11.5px; font-weight:bold; " class="testing">
                {{'Uzcard'}}
              </small> 
            </div>
...
export default {
  ...
  // created() {
  //   this.$root.$refs.payed_dolg = this;
  // },
  async mounted() {
  ...
  methods: {
    ...
    changingEnter(paytype,sum, id, fio, dolg_id){
      console.log('ichiga kirdingmi')
      console.log(paytype)
      ...
      this.zdachi = 0;
      this.client_id = id;
      this.client_name = fio;
      this.dolg_id = dolg_id;
      if(paytype==1){
        this.cashIn = parseFloat(sum)
        this.cashInString = new Intl.NumberFormat().format(sum)
      }
      else if(paytype==2){
        this.uzcardIn = parseFloat(sum)
        this.uzcardInString = new Intl.NumberFormat().format(sum)
      }
      else if(paytype==3){
        this.humoIn = parseFloat(sum)
        this.humoInString = new Intl.NumberFormat().format(sum)

      }
      else if(paytype==4){
        this.clickIn = parseFloat(sum)
        this.clickInString = new Intl.NumberFormat().format(sum)
      }
      else if(paytype==5){
        this.onlineIn = parseFloat(sum)
        this.onlineInString = new Intl.NumberFormat().format(sum)
      }
    },
    ...
    async fetchDolgPayClient(){
      let payed_summ = 0;
      payed_summ = parseFloat(this.clickIn) + parseFloat(this.uzcardIn) + parseFloat(this.humoIn) + parseFloat(this.cashIn)
      try{
        this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/PosClientDolgs/payClientDolg?pos_client_dolg_id=" + this.dolg_id + "&payed_summ=" + payed_summ);
        const data = await response.json();
        console.log('dolg')
        console.log(data)
        this.loading = false;
