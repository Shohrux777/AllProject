  ...
  <div class="addProductQtyPayed" >
    <div class="p-4 d-flex">
      ...
      <div class="client_list_pay" >
        ...
        <div v-show="!client_add_activ" class="client_list border">
      ...
      <div class="acceptBoxPayed px-3 py-2 text-center">
        <div class="client_info border-bottom">
          ...
          <div class="text-center" v-if="client_name">
            <span style="font-size: 16px; " class="mr-2">{{ $t('online_sum') }}:</span>
            <span style="font-size: 16.5px; " class="text-danger">{{ client_dolg }}</span>
          </div>
        </div>
        ...
        <div class="d-flex mt-2">
          <div class="skidka " style="width:35%">
            ...
            <div style="margin-top:15px;">
              ...
              <mdb-btn color="success" style="font-size:11px; padding: 6px 0;" @click="payed" class="m-0  mt-1 w-100" > {{'ok'}}</mdb-btn>
...
export default {
  ...
  methods: {
    ...
    async activ_client(i, data){
      ...
      await this.fetchClientIDDolg(data.id)
    },
      ...
      try{
        ...
        if(res.status == 200 || res.status == 200 ){
          ...
          for(let i=0; i<res_data.items_list.length; i++){
            dolg_sum += res_data.items_list[i].dolg_summ;
          }
          this.client_dolg = dolg_sum.toFixed(1).toString().replace(/(\d)(?=(\d{3})+(\.(\d){0,2})*$)/g, '$1 ')
        }
      }
      catch{
        this.modal_info = this.$i18n.t('network_ne_connect'); 
    ...
    async payed(){
      ...
      console.log(this.summa)
      var paysum = parseFloat(this.uzcardIn) + parseFloat(this.cashIn) + parseFloat(this.clickIn) + parseFloat(this.humoIn);
      var pay_dolg_summ = 0;
      if(parseFloat(paysum)>parseFloat(this.summa)){
        pay_dolg_summ = parseFloat(paysum)-parseFloat(this.summa)
      }
      ...
      // try{
        ...
        {
          console.log(data.id)
          this.getDiscoundAndDolg({skidka:this.clickIn, dolg:this.onlineIn, client_name: this.client_name, summa: paysum})

          this.updateCheckId(data.id)
          if(parseFloat(this.onlineIn)){
            await this.fetchDolgClient(data.id, this.client_id)
          }
          console.log('pay_dolg_summ')
          console.log(pay_dolg_summ)
          console.log(this.client_id)
          if(pay_dolg_summ>0){
            await this.fetch_payClientId_dolg(this.client_id, pay_dolg_summ)
          }
          this.$emit('print')
          this.$emit('close')
    ...
    async fetchDolgClient(check_id, client_id){
      console.log(check_id)
      console.log(client_id)
      console.log('client_id')
      let payed_summ = 0;
      payed_summ = parseFloat(this.clickIn) + parseFloat(this.uzcardIn) + parseFloat(this.humoIn) + parseFloat(this.cashIn)
      const requestOptions = {
        method : "POST",
        headers: { "Content-Type" : "application/json" },
        body: JSON.stringify({
          ...
          "real_company_id": localStorage.CompId,
          "dolg_summ": parseFloat(this.onlineIn),
          "id": 0,
          "posClientid": client_id,
          "posCheckid": check_id,
          // "posClientid": 0
        })
      };
    ...
    async fetch_payClientId_dolg(id, payed_sum){
      console.log('payed_sum')

      console.log(id)
      console.log(payed_sum)
      try{
        // this.loading = true;
        const response = await fetch(this.$store.state.hostname + "/PosClientDolgs/payClientDolgWithHistoryWithClietnId?pos_client_id=" + id + "&payed_summ=" + payed_sum);
        const data = await response.json();
        console.log(data)
        // this.loading = false;
        // if(response.status == 201 || response.status == 200)
