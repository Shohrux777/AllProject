<template>
    <div class="checked_show" >
      <div  class="ckeck_main_show  border">
        <div  style="text-align:center; margin: 10px 0;">
          <h5>Накладной № {{ options.id }}</h5>
        </div>
        <div class="infoHeader">
          <table style="width:100% !important">
            <tbody >
              <tr>
                <th  style="font-weight: bold;">Накладной №</th>
                <td class="text-right">{{options.id}}</td>
              </tr>
              <tr style="padding-top:10px;">
                <th  style="font-weight: bold;">Пользователь</th>
                <td class="text-right">{{options.posAuthorizationid}}</td>
              </tr>
              <tr style="padding-top:10px;" v-if="client_name != ''">
                <th class="paymentCheckPrintCheck" style="font-weight: bold;">Клиент</th>
                <td class="paymentCheckPrintCheck text-right" style="font-weight: bold;">{{client_name}}</td>
              </tr>
              <tr style="padding-top:10px;">
                <th style="font-weight: bold; padding-right: 35px;">Напечатан</th>
                <td class="text-right">{{options.created_date_time_str}}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="totalInfoShowCheck d-flex justify-content-between" style="border-top: 1px dashed black;">
          ...
          <h5 style="margin:0; padding:0 2px; font-weight:bold; font-size:13.5px;">{{parseFloat(parseFloat(options.summ) + parseFloat(options.online_sum))}} <span style="font-size:13px;">Сум</span></h5>
        </div>
        <!-- <div class="totalInfoShowCheck d-flex justify-content-between" style="border-top: 1px dashed black;" v-if="options.discount_summ">
          ...
          <h5 style="margin:0; padding:0 2px; font-weight:bold; font-size:13.5px;">{{options.discount_summ}} <span style="font-size:13px;">Сум</span></h5>
        </div> -->
        <div class="totalInfoShowCheck d-flex justify-content-between" style="border-top: 1px dashed black;" v-if="options.online_sum">
          ...
          <h5 style="margin:0; padding:0 2px; font-weight:bold; font-size:13.5px;">{{options.online_sum}} <span style="font-size:13px;">Сум</span></h5>
        </div>
  
        <div class="totalInfoShowCheck d-flex justify-content-between" style="border-top: 1px dashed black;">
          <h6 style="margin:0; padding:0 2px; font-weight:bold; font-size:14px;">К оплате</h6>
          <h4 style="margin:0; padding:0 2px; font-weight:bold; font-size:14px;" >{{parseFloat(options.summ)}} <span style="font-size:13.5px;">Сум</span></h4>
        </div>
