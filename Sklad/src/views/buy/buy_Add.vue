<template>
  <div class="invoice">
    <div class="barcode" v-if="open_barcode">
      <barcode @close="open_barcode = false" ref="barcodeInput"  :produc_id="search_product_id" />
    </div>
    <div class="" v-else>
      <loader v-if="loading"/>
            ...
            <div :class="{'applied': sklad_submitted}" class="container-fluid mt-3  bgColorBack p-4" style=" border-radius: 7px;">
              <mdb-row >
                ...
                <mdb-col col="3" style="position: relative;">
                  ...
                  <erpSelect
                    :options="allPostavchik.rows" :btnshow="false"
                    @select="select_postavchik"
                    :selected="postavchik_name"
                    :label="$t('company')"
                  />
                  <small class="" style="color: gray; position:absolute; top:-19px; left:20px; font-size: 12px;" >
                    {{$t('postavchik')}}
              ...
              <mdb-row class="mt-2">
                ...
                <small v-if="$v.time1.$dirty && !$v.time1.required" style="padding:90px 0 0 15px" class="invalid-text" >
                  ...
                  <mdb-col col="3" >
                    <mdb-input v-model="summa_product" size="sm" type="text" style="margin-top: 18px; margin-bottom: 0px; height: 32px;" :label="$t('summa_product')" />
                  </mdb-col>
                  <mdb-col col="3" >
                    <mdb-input v-model="given_summ" size="sm" type="text" style="margin-top: 18px;  margin-bottom: 0px; height: 32px;" :label="$t('given_summ')" />
                  </mdb-col>

                <mdb-col col="3" >
                <div class="ml-1" style="position:relative; margin-top: 0px;">
                        <input v-model="creditd_summ"  size="sm" type="number" 
                          style="margin-top: 18px; margin-bottom: 0px; background:#FAFAFA; height: 32px;  width:100%; color:red; outline:none; border:none; font-size:12px;"
                  ...
                  <mdb-tbl class="fixed-column" >
                    <mdb-tbl-head>
                      <tr>
                        ...
                        <td  v-show="false" style="min-width:100px; font-size: 13px;"> {{$t(datasource.columns[9])}} </td>
                    ...
                    <mdb-tbl-body>
                      <tr  scope="row" v-for="(row,rowIndex) in datasource.rows" :key="rowIndex" :width="row.size">
                        ...
                        <th>
                          <input v-model="row.soni" type="number" min="0"  step="0.1"  @input="onChangeCount(row.soni,rowIndex)"
                        ...
                        <th> <input v-model="row.unit" @input="onChangeCount(row.soni,rowIndex)" :disabled="row.numberDrag<=1" type="text" class="form-control form-control-sm"> </th>
                        <th> <input  v-model="row.numberDrag" disabled type="text" class="form-control form-control-sm"></th>
                        <th> <input v-model="row.price" @input="onchangePrice(row.soni,rowIndex)" type="text" class="form-control form-control-sm"> </th>
                        ...
                        <th  v-show="false"> <input  v-model="row.date" placeholder="12/22" @input="addComma(rowIndex)" type="text" class="form-control form-control-sm"> 
                          <small class="invalid-text" style="margin:0; margin-top:26px; position:relative; "
                            v-if="($v.datasource.rows.$each[rowIndex].date.$dirty  && !$v.datasource.rows.$each[rowIndex].date.required)" >
                              {{$t('Select_date')}}
                          </small>
                        </th>
                        <th > <input v-model="row.optom_price"  type="number" class="form-control form-control-sm" >  </th>
                        
                        <th class="text-center" v-show="false">
  ...
  validations: {
    ...
    data(){
      return{
        ...
        invoice_name: '',
        search_product_name:'',
        search_product_id: null,
        open_barcode: false,

        product_add_status : true,
    ...
    async mounted(){
      ...
      await this.fetchPostavchik();
      // if(this.allPostavchik.rows.length>0){
      //   this.postavchik_name = this.allPostavchik.rows[0].name;
      //   this.postavchik_id = this.allPostavchik.rows[0].id;
      // }
    ...
    computed:mapGetters(['allSklad', 'allDepartment', 'allCompany', 'allProduct', 'allPostavchik']),
     methods : {
      ...mapActions(['fetchDepartment','fetchSklad','fetchCompany', 'fetchProduct', 'fetchPostavchik']),
      ...mapMutations(['get_deparment_id_comp']),
      selectSearchProduct(option){
        console.log(option)
        this.search_product_name = option.data.name;
        this.search_product_id = option.data.id;
        // this.$router.push('/barcode/' + option.data.id)
        this.open_barcode = true;
        // this.$refs.barcodeInput.anything();
      },
     ...
     async select_comp(option){
      ...
      select_postavchik(option){
        this.postavchik_name = option.name;
        this.postavchik_id = option.id;
      },
      add_product(data)
      {
     ...
     async close_product(last_product)
      {
        ...
        if(this.last_row != -1)
        {
          this.datasource.rows[this.last_row].product_name = last_product.name.name;
          this.datasource.rows[this.last_row].main_item_id = last_product.name.id;
          this.datasource.rows[this.last_row].product_id = last_product.name.id;
          this.datasource.rows[this.last_row].numberDrag = last_product.name.contains_number_in_pack;
          this.datasource.rows[this.last_row].buyed_price = last_product.name.buyed_price;
          this.datasource.rows[this.last_row].price = last_product.name.price;
          this.datasource.rows[this.last_row].prosent = last_product.name.percentage;
          this.datasource.rows[this.last_row].optom_price = last_product.name.optom_price;
        }
      ...
      selectproduct(option)
      {
        ...
        if(option.data.optom_price == '' || option.data.optom_price == null){
          this.datasource.rows[option.row].optom_price = 0;
        }
        else{
          this.datasource.rows[option.row].optom_price = option.data.optom_price;
        }
      ...
      onChangeCount(data,index)
      {
        ...
        console.log('this.datasource.rows[index]')
        console.log(this.datasource.rows[index])
        if(this.datasource.rows[index].numberDrag>1){
          this.datasource.rows[index].summ += parseInt(parseFloat(this.datasource.rows[index].price)*parseFloat(this.datasource.rows[index].unit)/parseFloat(this.datasource.rows[index].numberDrag))
        }
        this.summa_product = 0;
        for(let i=0; i<this.datasource.rows.length; i++){
          this.summa_product +=this.datasource.rows[i].summ;
        }
      },
      onchangePrice(data,index){
        this.datasource.rows[index].summ = data* this.datasource.rows[index].price;
        if(this.datasource.rows[index].numberDrag>1){
          this.datasource.rows[index].summ += parseInt(parseFloat(this.datasource.rows[index].price)*parseFloat(this.datasource.rows[index].unit)/parseFloat(this.datasource.rows[index].numberDrag))
        }
      ...
      async submit()
      {
        ...
        for(var i = 0; i < this.datasource.rows.length; i++){
          ...
          a.persantage_nds = parseInt(this.datasource.rows[i].prosentNDS);
          ...
          if(this.datasource.rows[i].optom_price == '' || this.datasource.rows[i].optom_price == null){
            a.optom_price = 0;
          }
          else{
            a.optom_price = this.datasource.rows[i].optom_price;
          }
          ...
          try{
            ...
            // }
            if(parseFloat(this.given_summ)>parseFloat(this.summa_product)){
              swq = Math.abs(parseFloat(this.given_summ) - parseFloat(this.summa_product))
            }
            console.log(swq)
         const requestOptions = {
            ...
            body: JSON.stringify({
              "real_company_id": parseInt(localStorage.CompId),
              "date": this.time1,
              "postavshik_id" : this.postavchik_id,
              "sklad_id" : this.sklad_id,
              "department_id": this.dept_id,
              "factura_number": swq.toString(),
              "debit_summ": this.given_summ,
              "summ": this.summa_product,
              "credit_sum": this.creditd_summ,
              "note" : this.note + ' (' + this.postavchik_name +  ')',
           ...
           if(response.status == 200 || response.status == 201)
            {
              ...
              if(swq != 0){
                await this.fetchCreditorPay(swq);
              }
              // if(this.creditd_summ > 0){
              //   await this.fetchCreditorAdd(this.creditd_summ, data.id);
              // }
      ...
      async fetchCreditorAdd(dolg, invoice_id){
        try{
         const requestOptions = {
            method : "POST",
            headers: { "Content-Type" : "application/json" },
            body: JSON.stringify({
              "creditor_price": parseFloat(dolg),
              "real_creditor_price": parseFloat(dolg),
              "posInvoiceid" : invoice_id,
            })
          };
            console.log('requestOptions.body')
            console.log(requestOptions.body)
          this.loading = true;
           const response = await fetch(this.$store.state.hostname + "/PosCreditor", requestOptions);
          this.loading = false;
           if(response.status == 200 || response.status == 201)
            {
              const data = await response.json();
      ...
      async fetchCreditorPay(payed_summ){
        try{
          this.loading = true;
           const response = await fetch(this.$store.state.hostname + "/PosCreditor/payForCreaditorCompanyByPostavshikId?postavshik_id=" + this.postavchik_id + '&payment_summ_all=' + payed_summ);
          this.loading = false;
           if(response.status == 200 || response.status == 201)
            {
              const data = await response.json();
          ...
          this.note= data.note;
           ...
           for(var i = 0; i<data.invoiceItems.length; i++)
           {
              this.datasource.rows.push({
                ...
                optom_price:data.invoiceItems[i].optom_price,
