<template>
  <div>
    ...
    <div class="mx-3 mt-3 py-1 p-0 border-bottom">
      <mdb-row>
        ...
        <mdb-col col="8" style="position: relative;">
          <div class="text-right" style="margin-top: -3px;">
            <mdb-btn :disabled="!client_id" @click="show_check = true" color="success" size="sm">Платить долг</mdb-btn>
          </div>          
        </mdb-col>
      </mdb-row>
    </div>
      ...
      <div class="row mt-3 border-bottom pb-3">
          <div class="col-4">
              <div class="qty borderSolder py-2">
                  <span class="ml-3">Цена полученного товара</span>
          ...
          <div class="col-4">
              <div class="qty borderSolder py-2">
                  <span class="ml-3">Выплаченная сумма</span>
                  <div class="text-right  px-3 mt-1">
          ...
          <div class="col-4">
              <div class="qty borderSolder py-2">
                  <span class="ml-3">Баланс долга</span>
                  <div class="text-right px-3 mt-1">
      ...
      <div class="col-7">
        <div class="card px-4 py-2 mb-2" v-for="(item,i) in client_summa_list" :key="i">
          <div class="d-flex justify-content-between border-bottom m-0">
            <p class="m-0 mb-1" style="font-size: 12.5px;"><span class="mr-3">{{ $t('date') }}:</span>{{item.created_date_time.slice(0,10)}} 
              <span class="ml-1 font-weight-bold">{{item.created_date_time.slice(11,16)}}</span>
            </p>
          ...
          <div class="check_table" v-if="item.invoiceItems.length">
            <table style="width:100% !important">
              ...
              <tbody>
                <tr v-for="(data,index) in item.invoiceItems" :key="index" style="border-bottom: 0.5px solid #CED4DA;">
                  <td style="font-size: 12px;">{{data.product.sale_name}}</td>
                  ...
                  <td style="text-align: right; font-size: 12px;">{{data.unit_buyed_price}}</td>
                  <td style="text-align: right; font-size: 12px;">{{data.summ}}</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div class="d-flex justify-content-between border-bottom m-0 mt-2" v-if="parseFloat(item.summ) != 0">
            <p class="m-0 mb-1" style="font-size: 12.5px;"> 
              {{ $t('total') }}
            </p>
            <p class="m-0 mb-1 text-dark" style="font-size: 12.5px;"> {{ item.summ}}</p>
          </div>
          <div class="d-flex justify-content-between border-bottom m-0 mt-2">
            <p class="m-0 mb-1" style="font-size: 12.5px;"> 
              К оплате
            </p>
            <p class="m-0 mb-1 text-success" style="font-size: 12.5px;"> + {{ item.debit_summ}}</p>
          </div>
          <div class="d-flex justify-content-between border-bottom m-0 mt-2" v-if="item.credit_sum>0">
            <p class="m-0 mb-1" style="font-size: 12.5px;"> 
              {{ $t('online_sum') }}
            </p>
            <p class="m-0 mb-1 text-danger" style="font-size: 12.5px;"> - {{ item.credit_sum}}</p>
          </div>
          <div class="d-flex justify-content-between border-bottom m-0 mt-2" v-if="parseFloat(item.factura_number)>0">
            <p class="m-0 mb-1" style="font-size: 12.5px;"> 
              {{ $t('pay_dolg_id') }}
            </p>
            <p class="m-0 mb-1 text-secondary" style="font-size: 12.5px;"> + {{ parseFloat(item.factura_number).toFixed(1)}}</p>
          </div>
        </div>           
      </div>
      <div class="col-5 ">
        <div class="card px-4 py-2 mb-2" v-for="(dolg,i) in client_dolg_list" :key="i" v-show="dolg.real_creditor_price >0.1">
          <div class="d-flex justify-content-between border-bottom m-0">
          ...
          <div class="d-flex justify-content-between border-bottom m-0 mt-2">
            ...
            <p class="m-0 mb-1 text-danger font-weight-bold" style="font-size: 12.5px;"> - {{ dolg.real_creditor_price.toFixed(1)}}</p>
          </div>
        </div>        
      </div>
    </div>
    ...
    <mdb-modal size="lg" :show="show_check" @close="show_check = false">
      <mdb-modal-header>
        <mdb-modal-title>Платить долг</mdb-modal-title>
      </mdb-modal-header>
      <mdb-modal-body>
        <div class="container-fluid">
          <mdb-input class="m-0 p-0"  v-model="client_name" size="md"  outline  group type="text" validate error="wrong" disabled success="right"/>
          <mdb-input class="m-0 p-0" v-model="payed_summa" size="md"  outline  group type="number" validate error="wrong"  success="right"/>

        </div>
      </mdb-modal-body>
      <mdb-modal-footer>
        <mdb-btn color="secondary" size="sm" @click.native="show_check = false">Close</mdb-btn>
        <mdb-btn color="primary" @click="payed_dolg_postavchik" size="sm">Платить</mdb-btn>
      </mdb-modal-footer>
    </mdb-modal>
...
export default {
  ...
  computed: {
    ...mapGetters(['allProduct', 'allClient', 'allPostavchik', 'allDepartment', 'allSklad']),
  ...
  methods: {
    ...mapActions(['fetchProduct', 'fetchClient', 'fetchPostavchik', 'fetchDepartment', 'fetchSklad']),
    ...mapMutations([ 'update_pagination_first']),
    async selectClient(option){
      console.log(option)
      this.client_name = option.name;
      this.client_id = option.id;
      await this.fetchClientIDDolg(option.id)
      await this.fetchClientICheck(option.id)
    },

    async fetchClientIDDolg(id){
      try{
        const res = await fetch(this.$store.state.hostname + '/PosCreditor/getPaginationPosCreditorNotPaidListByPostavshikId?page=0&size=100&postavshik_id=' + id);
        ...
        if(res.status == 200 || res.status == 200 ){
          ...
          for(let i=0; i<res_data.items_list.length; i++){
            this.client_dolg += res_data.items_list[i].real_creditor_price;
          }
    ...
    async fetchClientICheck(id){
      try{
        const res = await fetch(this.$store.state.hostname + '/PosInvoices/getPaginationByPostavshikIdFull?page=0&size=100&postavshik_id=' + id);
        const res_data = await res.json();
        console.log('res_data');
        console.log(res_data);
        ...
        if(res.status == 200 || res.status == 200 ){
          ...
          this.client_summa = 0;
          for(let i=0; i<res_data.items_list.length; i++){
            this.client_total += res_data.items_list[i].summ;
            this.client_summa += res_data.items_list[i].debit_summ;
          }
    ...
    // },
      ...
      async payed_dolg_postavchik(){
        if(this.payed_summa == 0 || this.payed_summa == null || this.payed_summa == ''){
          return;
        }

        await this.fetchCreditorPay(this.payed_summa)
        await this.fetchCreditorPayInvoice()
      ...
      async fetchCreditorPay(payed_summ){
        try{
          this.loading = true;
           const response = await fetch(this.$store.state.hostname + "/PosCreditor/payForCreaditorCompanyByPostavshikId?postavshik_id=" + this.client_id + '&payment_summ_all=' + payed_summ);
          this.loading = false;
        ...
        try{
         const requestOptions = {
            ...
            body: JSON.stringify({
              ...
              "postavshik_id" : this.client_id,
              "sklad_id" : this.sklad_id,
              "department_id": this.dept_id,
              "factura_number": this.payed_summa.toString(),
              "debit_summ": this.payed_summa,
              "summ": 0,
              "credit_sum": 0,
              "note" : ' Qarz tulandi',
              "id": 0
           ...
           if(response.status == 200 || response.status == 201)
            {
              const data = await response.json();
              console.log(data)
              await this.fetchClientIDDolg(this.client_id)
              await this.fetchClientICheck(this.client_id)
              this.payed_summa = 0;
              this.$refs.message.success('Added_successfully')
              // this.$store.state.alert = true;
            }
